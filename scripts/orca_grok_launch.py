"""Grok 워커를 다른 CLI 워커와 같은 방식으로 터미널에 붙이는 런처.

Grok CLI 를 Orca 워커로 씁니다. 터미널을 런처를 명령으로 지정해 먼저 만들고,
런처는 preamble 파일이 나타날 때까지 기다렸다가 grok 을 기동합니다.

대화형 모드는 grok [PROMPT] 위치 인자로 기동하며 os.execvpe 로 제어권을 넘깁니다.
...
대화형 셸로 이어받아 터미널 창을 유지합니다.
    ...
    orca terminal create --worktree path:<워크트리> --title "<섹션명>" \
      --command "uv run python scripts/orca_grok_launch.py --model grok-2"
...
import argparse
import os
import subprocess  # nosec B404 - 코디네이터가 만든 고정 인자 목록으로만 grok 을 호출합니다
import sys
...
def build_command(
    ...
    *,
    ...
    """grok 기동 명령 배열을 조립합니다.

    대화형은 grok [PROMPT] 위치 인자이고 단발은 -p 입니다. 모델은 -m 또는 --model 입니다.
    권한 자동 승인 인자(--always-approve)는 기본으로 붙이지 않으며 명시적으로 요청될 때만 포함합니다.
    """
    cmd = ["grok", "--model", model]
    if always_approve:
...
def build_completion_message(exit_code: int, model: str) -> str:
    """grok 종료 후 터미널에 남길 완료 안내 문구를 조립합니다."""
    return f"\n---\nGrok 작업 완료 (종료 코드: {exit_code})\n---\n"
...
def run_grok(cmd: list[str], env: dict[str, str]) -> int:
    """grok 을 자식 프로세스로 실행하고 표준 입출력은 터미널에 그대로 둡니다."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Grok 워커 런처")
    parser.add_argument(
        "--model",
        required=True,
        help="Grok 모델 ID (예: grok-2, grok-beta)",
    )
    ...
    parser.add_argument(
        "--always-approve",
        action="store_true",
        help="권한 자동 승인 (--always-approve) 플래그를 추가합니다. 기본값은 미사용입니다.",
    )
    parser.add_argument(
        "--role",
    ...
    print(f"기동: grok --model {args.model} ({mode}, 지시문 {len(prompt)}자)", flush=True)

    if args.one_shot:
        exit_code = run_grok(cmd, env)
