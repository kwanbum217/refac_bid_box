...
| `quantile` alpha 조정 (0.35~0.58) | 0.45 가 꼭짓점인 U 자형으로 홀드아웃에서 여섯 지표가 모두 개선(MAE 1.2407 -> 1.2323)됐으나 운영 9,997건 쌍대에서 MAE t=-0.44 판별 불가, 적중률은 63.99% -> 63.92% 로 뒤집힘. RMSE 만 t=-5.60 유의했고 단독 승격 불가. **alpha 를 올리는 방향(0.52~0.58)은 단조 악화** | `servc_quantile_alpha_20260810.md` |
...
### 홀드아웃 개선이 운영으로 옮겨지지 않으면 특징 경로를 의심하십시오

alpha 실험에서 홀드아웃 MAE 차이 -0.0083 이 운영에서 -0.00171 로 5분의 1이
됐고 적중률은 부호까지 뒤집혔습니다. 조기 종료는 이번에 맞춰져 있었습니다.

**단서는 두 경로의 절대 MAE 입니다.**

    홀드아웃 (parquet 특징)    2025년 MAE 1.2407
    운영     (서빙 특징)       2025년 MAE 1.4109

같은 해 같은 모델인데 0.17 차이입니다. 표본 크기로 설명되지 않습니다.

    학습    attach_institution_history 가 누수 없이 계산한 inst_hist_rate
    서빙    InstitutionWinRateStat 사전 집계 표 조회

두 값이 어긋나면 홀드아웃의 미세한 개선은 운영에서 잡음에 묻힙니다.
2026-08-05 에 `inst_sample_cnt` 가 서빙에서 항상 0 이던 결함을 고쳐 운영 MAE 를
6.1% 낮춘 전례가 있습니다(`servc_feature_parity_20260805.md`).

**목적함수를 더 다듬기 전에 특징 값 대조를 다시 하십시오.** 옮겨지지 않는
이득의 원인이 거기 있을 수 있습니다.
