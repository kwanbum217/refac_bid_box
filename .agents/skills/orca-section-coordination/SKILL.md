...
**상시 감시 자동 기동은 워커 기동 절차의 필수 강제 조항입니다.** `scripts/orca_taskctl.py dispatch` 는 워커 기동 성공 시 상시 감시기(`scripts/orca_worker_watch.py --watch --respawn`)를 배경 프로세스로 자동 기동하며, 이미 실행 중이면 단일 인스턴스로 재사용합니다. 무제한 `--watch` 는 차단을 출력한 뒤에도 종료하지 않습니다. 한 워커의 회수 대기나 승인 대기가 다른 워커 감시를 끊지 않게 하기 위함입니다. 또한 권한 자동 승인 감시기 부착 실패 시 `dispatch` 는 fail-closed 원칙에 따라 기본값에서 즉시 거부(종료 코드 2)합니다. 의도적 우회는 명시 플래그(`--skip-auto-approve-check`) 지정 시에만 허용되며 경고가 남습니다.
...
워커 없이 코디네이터가 직접 쓴 브랜치도 병합 전에 최소 Capsule 을 만들어 게이트 2(쓰기 범위)와 게이트 3(검증 능력)을 강제합니다. 2026-09-23 커밋 `b05dc173` 은 빌더 Capsule 이 없어 게이트 2·3 이 skip 으로 건너뛰어졌습니다.
