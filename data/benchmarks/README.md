...
`data/benchmarks/` 의 원시 JSON 은 보고서에 적은 수치를 다시 계산할 수 있는 근거이므로 보존합니다.

- 원시 JSON 은 보고서 수치의 재계산 근거이므로 지우지 않습니다.
- 새 원시 결과는 그 결과를 인용하는 분석 문서와 같은 커밋에 넣습니다.
- 클론 비용이 문제가 되면 Git LFS 나 아카이브 분리를 별도 결정으로 검토합니다.

본 정책은 원시 결과를 옮기거나 지우는 것을 허용하지 않으며, 보존 위치 변경이 필요하면 별도 결정을 거칩니다.
