...
16건 중 **6개 필드, 7건**이 2025-01 에 몰려 있습니다: `arsltApplDocRcptMthdNm`, `bidPrtcptFee`, `ntceInsttOfclEmailAdrs`, `pqEvalYn`, `tpEvalYn`, `rsrvtnPrceReMkngMthdNm`. 이미 확인된 세 필드와 합치면 **9개 필드가 2025-01 을 경계로 계단식으로 움직입니다.**
...
이전 감사(`servc_2025_source_regime_shift_20260811.md`)에서 학습 사용 필드 중 유일하게 전환이 확인된 것은 `sucsfbidLwltRate`(하한율) 하나입니다. 이번 확대 조사로 **학습에 쓰는 13개 필드 전부(`IN_USE`)를 2025-01 경계 관점에서 훑은 결과, 하한율 외에는 없다**는 것이 확정됐습니다. 하한율의 결측 성격 재검토(선행 문서 4.1)가 여전히 가장 중요한 다음 작업입니다.
