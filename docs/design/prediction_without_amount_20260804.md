...
원본 `bids/detail.html` 의 오류 처리가 그대로 동작합니다. 422 응답의 `detail` 이
알림창에 표시됩니다. 템플릿은 수정하지 않았습니다.
...
외자 공고는 절반이 예측 불가입니다. 화면에서 예측 버튼 자체를 비활성화하고
사유를 미리 보여줄지, 지금처럼 눌렀을 때 알리는지는 UX 판단이 필요합니다.
현재는 원본 화면을 그대로 두는 쪽을 택했습니다.
