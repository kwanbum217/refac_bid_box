...
## 15. 예측 구간 피복률 검증 (2026-08-04)

6.3 절이 정한 예측 구간이 실제로 맞는지 2025년 홀드아웃으로 측정했습니다.
결과는 별도 문서에 있습니다.

[`servc_prediction_interval_20260804.md`](servc_prediction_interval_20260804.md)

요점은 **소박한 분위 회귀 구간을 그대로 노출하면 안 된다**는 것입니다.
명목 80% 구간의 실제 피복률이 75.52% 이고, 10억 이상 구간에서는 66.85% 까지
떨어집니다. 등각예측 보정을 함께 적용해야 목표 90% 에서 88.56% 가 나옵니다.

미적용 상태이며 운영 경로에 배선하지 않았습니다.
