...
> **작성일**: 2026-08-03
> **버전**: v1.2.0
> **상태**: 제도 조사 완료. 데이터 검증 완료(V1~V7). **모델 실험 완료, 8장 일부 철회**
...
> **실험 보고**: [`docs/design/servc_segment_experiment_20260803.md`](servc_segment_experiment_20260803.md)
>
...
> **철회 (v1.2.0)**: 아래 분리 설계는 **실측으로 기각되었습니다.**
> 세 세그먼트(수의계약 0.7207 → 0.7171, 경쟁_공고서참조 0.3831 → 0.3826,
> 협상 0.2006 → 0.1930) 전부에서 분리가 단일보다 나빴고, 전체도
> 0.6683 → 0.6659 로 악화했습니다. **범주형 특징을 넣은 시점에 트리가 이미
> 세그먼트를 학습하며, 분리는 학습 표본만 줄입니다.**
> 근거는 [분리 모델 실험 보고서](servc_segment_experiment_20260803.md) 3장입니다.
>
> **확정 설계는 단일 모델 + 범주형 제도 특징입니다.** 아래 도식은 세그먼트별
> 난이도 차이를 보여주는 참고 자료로만 남깁니다.

검증 결과 최상위 분기 후보는 낙찰방식이 아니라 계약방법이었습니다.
표본의 50.4% 가 수의계약이기 때문입니다 (검증 보고 2장).
...
> **격하 (v1.2.0)**: 하한 절단을 "필수 개선책"에서 **안전장치**로 내립니다.
> 실측 개선이 R2 +0.0001 (0.6683 → 0.6684) 에 그쳤습니다. `lwlt_rate` 를 특징으로
> 넣은 시점에 모델이 이미 하한율 위쪽을 예측하므로 절단할 대상이 거의 없습니다.
>
> **제거하지는 않습니다.** 비용이 0 이고 물리적으로 불가능한 예측을 막아 줍니다.
> 다만 이것으로 성능이 좋아진다고 주장해서는 안 됩니다.
> 근거는 [분리 모델 실험 보고서](servc_segment_experiment_20260803.md) 2장입니다.
