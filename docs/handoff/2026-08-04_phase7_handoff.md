...
| G3 예측 API P95 100ms | 통과. 2026-08-06 재측정 19.1ms (기동 직후 100회, 동시성 10) |
| G3 SSE 전체 P95 20초 | 통과 (2026-08-06 6.39s) |
| G3 SSE 첫 토큰 P95 3초 | 통과. 2026-08-06 2.66s |
| G2 Windows 검증 | CI 3개 작업 전량 통과 (2026-08-06). Docker Compose 전체 스택은 미수행 |

**2026-08-06 갱신.** 예측 API 는 기동 직후 P95 164.1ms 로 목표를 넘겼습니다.
꼬리 전부가 첫 요청들이 문 모델 로드 비용이었고(예열 뒤 같은 부하에서 16.4ms),
기동 시 배경 예열을 붙여 19.1ms 로 내렸습니다. 47행의 1.4ms 는 이미 예열된
서버를 잰 값이라 배포 직후 사용자가 겪는 값이 아니었습니다.
...
당시 P95 11.06s, 목표 3s. 아래는 그 시점의 판단이며 2026-08-05 에 정정됐습니다.
...
> **2026-08-05 정정.** 위 진단은 틀렸습니다. "LLM 첫 토큰까지" 안에 무엇이
> 들어 있는지 쪼개지 않은 채 프리필로 단정했습니다. 실제 원인은 `gemma4` 의
> 사고(thinking) 단계였습니다. 사고 토큰은 0.57초에 나오는데 우리가 읽는
> `message.content` 는 사고가 끝난 뒤에야 시작됩니다. `think: false` 로
> 첫 토큰이 10.53초에서 0.38초가 됐고, 모델 상주(`keep_alive`)로 콜드 로드
> 11.78초를 없앴습니다. **로컬 LLM 을 유지한 채 목표가 사정권에 들어왔으므로
> 아래 선택지는 더 이상 필요하지 않습니다.** 근거:
> [`docs/design/sse_first_token_20260805.md`](../design/sse_first_token_20260805.md)

선택지 (2026-08-05 이후 불필요. 기록으로만 보존):
