...
### 4.1.1 RAG 다음 단계 (2026-08-06 추가)

임베딩을 bge-m3 로 교체해 검색이 비로소 동작합니다. **되돌리지 마십시오.** 근거는 [`rag_embedding_korean_20260806.md`](../design/rag_embedding_korean_20260806.md) 이며, 옛 "임베딩 교체 금지" 기록은 폐기됐습니다(근거였던 19개 컬렉션이 실제로는 1개, 원본은 스냅샷 보존).

| top-5 적중률 | 문서 간 유사도 0.9 이상 |
| --- | --- |
| 4.0% -> **100.0%** | 60.5% -> **0.1%** |

다음 과제는 **KB 커버리지 확대**입니다. 현재 운영 색인은 500건인데 최근 1년 공고는 499,467건으로 **0.1%** 만 담고 있습니다. `KB_MAX_DOCUMENTS` 로 조절합니다.

| 규모 | 적재 | 디스크 |
| --- | ---: | ---: |
| 1만 건 | 3.6분 | 72MB |
| 10만 건 | 약 36분 | 0.70GB |
| 50만 건 (전량) | 약 3시간 | 3.49GB |

증분 색인이 들어가 있어 초기 적재만 한 번 비싸고 이후 야간 재색인은 변경분에 비례합니다([`kb_incremental_indexing_20260806.md`](../design/kb_incremental_indexing_20260806.md)).

확대 후에는 같은 방법으로 적중률을 다시 재십시오. 후보가 늘면 순위가 밀릴 수 있습니다. 측정 스크립트 방식은 설계서 2장에 있습니다(정답을 아는 질의 100건, 시드 42).

**원문(HWP/PDF) 수집·청킹·overlap 은 그다음입니다.** 현재 KB 문서는 DB 행에서 조립한 정형 레코드(평균 183자)라 쪼갤 대상이 없습니다.
...
수의시담 계열 MAE 2.36 대 적격심사 계열 0.79로 난이도 차이가 큽니다. 다만
낙찰방법별 분리 모델은 이미 기각됐습니다(분리 R2 0.6659, 단일 R2 0.6683).
분리 학습을 다시 시작하지 말고 결측 집단의 잔차가 연도별로 안정적인지 먼저
진단해야 합니다. 모델 전용 재개 절차는
[`2026-08-06_model_improvement_handoff.md`](2026-08-06_model_improvement_handoff.md)를
따릅니다.
