...
| 대상 | 상태 |
| --- | --- |
| 워커 터미널 5개 | **종료 완료** (cursor, Antigravity, kimi 및 실패 잔재 2개) |
| 워크트리 4개 | **제거 완료**. 주 저장소 하나만 남았습니다 |
| 병합 완료 브랜치 14개 | **삭제 완료**. 전부 `git branch -d` 로 지웠고 `-D` 는 쓰지 않았습니다 |
| `kwanbum217/orca-w1-concurrency-2` | **보존**. 유일한 미병합 세션 브랜치이며 기각 판정 근거(`12f88c3`)가 담겨 있습니다 |
| 베이크오프 9개, `integrate/arq-worker-cutover`, `feat/*`, `perf/predict-tail` | **보존**. 판정은 끝났으나 삭제는 다음 세션 결정 사항입니다 |
...
| # | 과업 | 상태 | 차단 요인 | 소요 |
| :---: | --- | --- | --- | --- |
| 1 | **LLM fixture 밖 일반화 측정** | 미착수 | 없음 | 3~5시간 |
| 2 | **numeric 절대 정확도 개선** (현재 65.7%) | 미착수 | 없음 | 미상. 탐색 과제 |
| 3 | 회수 후보 2건 반영 여부 결정 | 판정 완료, 결정 대기 | 사용자 판단 | 1시간 |
| 4 | 폐기 권고 브랜치 삭제 | 판정 완료 | 사용자 승인 | 10분 |
| 5 | **G3 컷오버 최종 판정** | 대기 | 1·2 및 6 | 판정만 1시간 |
| 6 | Windows Docker Desktop 실기 | **불가** | **장비 부재** | - |

**1번과 2번이 유일하게 지금 착수 가능한 실질 과업입니다.** 3·4번은 판정이 이미
끝나 실행만 남았고, 5번은 앞의 것들이 닫혀야 하며, 6번은 장비가 생기기 전에는
움직일 수 없습니다.

### 6.2 착수 권고 순서와 근거

**1번을 먼저 하십시오.** 2026-08-26 v4 측정으로 `gemma4:e2b` 를 승격했으나 그
근거는 24문항 fixture 안에서만 성립합니다. e2b 는 더 작은 모델이므로 **fixture
밖에서 뒤집힐 수 있고, 그 상태로 컷오버하면 운영에서 발견**하게 됩니다. 2번은
품질 상한을 올리는 개선이지만 지금 잘못된 판정이 서 있지는 않습니다.

측정 주의사항입니다.

- **측정 중 저장소를 동결하십시오.** 2026-08-26 에 측정 중 병합으로 72회차를 전량 폐기한 적이 있습니다.
- fixture 를 새로 만든다면 **q20~q24 refusal 8문항 구조**를 유지하십시오. 과잉응답·과잉거절 채점이 거기 걸려 있습니다.
- 지연 정본 판정은 `benchmark_rag_segments.py` 가 담당합니다. 품질 측정 스크립트의 지연 값을 정본으로 쓰지 마십시오.

### 6.3 3·4번 실행 지침

**3번 회수 후보 2건.** `b2-or_nemoultra` 의 `--json` 출력과 `bakeoff-mimo` 의
`audit_with_state` 분리입니다. 회수하기로 하면 **브랜치를 병합하지 말고 해당
변경만 새 브랜치로 옮기십시오.** 두 브랜치 모두 2026-08-20 기준이라 그 뒤
`audit_model_inventory.py` 변경과 충돌합니다.

**4번 폐기 대상.** 베이크오프 7개와 `integrate/arq-worker-cutover` 입니다.
`feat/codex-task-routing`(2026-08-14 전량 폐기 권고)과
`perf/predict-tail`(2026-08-14 병합 불가)도 같은 상태입니다.
`kwanbum217/orca-w1-concurrency-2` 는 3.1 절 기각 근거이므로 **함께 지우지
마십시오.** 삭제 전 `git log --oneline main..<branch>` 를 반드시 읽으십시오.

### 6.4 컷오버 판정 시 주의

개별 게이트 PASS 를 컷오버 PASS 로 승격하지 마십시오. 6번이 장비 부재로 영구
미해결일 가능성이 있으므로, **컷오버 조건에서 Windows 실기를 어떻게 다룰지가
먼저 결정되어야 합니다.** 이것은 기술 판단이 아니라 사용자 결정 사항입니다.
