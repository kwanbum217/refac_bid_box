...
> **상태**: 하이퍼파라미터 **기각**, 앙상블 **여지 작음**, 교차 이력 **기각**(2026-08-09). 사전 순위 축이 모두 닫혔습니다
...
## 3. LightGBM + CatBoost 앙상블 (여지 작음. 강등)
...
**CatBoost 에 L1 계열 목적함수(`MAE` 또는 `Quantile:alpha=0.5`)를 주고 다시
재는 것은 아직 안 해 봤습니다.** 지금 결과는 목적함수가 서로 다른 두 모델을
섞은 것이라, 맞춘 뒤에도 같은 결론일지는 모릅니다.

다만 우선순위는 낮습니다. 상관 0.9389 는 한계에 가깝고, 목적함수를 맞추면
상관은 더 올라갈 가능성이 큽니다. 착수한다면
`eval_servc_ensemble_headroom.py` 에 CatBoost 하이퍼파라미터 인자를 더해
같은 판정을 다시 내는 것으로 시작하십시오.

`train_and_register` 의 선택 로직을 손대는 것은 **그 측정이 긍정적일 때만**
하십시오. 범위가 큽니다.
...
**모든 사전 순위 축이 닫혔습니다.** 미측정으로 남은 갈래는 3.2 의 CatBoost
L1 목적함수 하나이며, 잔차 상관 0.9389 가 이미 한계에 가깝고 목적함수를 맞추면
상관은 더 오를 가능성이 커 기대값이 낮습니다.
