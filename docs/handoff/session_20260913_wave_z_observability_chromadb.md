...
### 4.2 chromadb (만료 2026-12-31): 사전 조사 완료, 실행 미착수
...
실행 전에 **사용자 결정 두 가지**가 필요합니다.

| 결정 | 이유 |
| --- | --- |
| 후보 B 승인 | 3.6G 복사본과 서비스 중지 창이 필요합니다 |
| 1.x 목표 버전 | CVE 세 건이 수정된 최소 버전이 미확인입니다 (보고서 UNK-03) |

보고서 6.2 절의 미확인 세 건(UNK-01 HNSW 인덱스 제자리 변환, UNK-02 `verify_migration.py` 직접
SQL 호환, UNK-03 수정 버전)은 스테이징 복사본 실측으로만 닫힙니다. **원본 `chroma_db/` 를 1.x 로
열지 마십시오.**
...
| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | chromadb 후보 B 승인과 목표 버전 결정 | 사용자 결정 |
| 2 | 스테이징 복사본에서 UNK-01~03 실측 | 1 |
| 3 | 셸 명령 자동 승인 실동작 확인 | 다음 워커 기동 |
| 4 | Windows 실기 검증 | 장비. G2 유일 잔여 조건 |
