...
3. **`d822e88`**: 다중 스레드 CPU 계측 정확도를 위한 `thread_time` 도입, 기계 파싱 포맷 통일
4. **`93158cd`**: `uvicorn.error` 로거 가시성 교정, 벤치마크 원시 JSON 4건 적재, 문서 갱신

## 후속 다중 워커 판정

실제 Docker 3·4워커 재검증은 모두 c10 P95 100ms 목표에 실패했습니다. Compose
기본값은 1로 복원했고 다중 워커는 수동 실험 옵션으로만 남겼습니다. 최종 수치와
기각 근거는
