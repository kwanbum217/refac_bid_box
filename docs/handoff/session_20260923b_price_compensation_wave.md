...
| 예정가격이 없고 배점표가 완비된 공고를 `PRED_PRICE_UNAVAILABLE` 차단 대신 `rate_only` 성공 응답으로 바꾸는 변경, 그리고 기초금액이 없을 때 `candidate_bid_amount` 를 1 로 보내는 화면 변경 | 응답 계약이 바뀌고 `test_bid_without_pred_price_is_blocked` 의 기대가 뒤집힌다. 도메인 함수는 `rate_only` 를 이미 지원한다 | **기각(2026-09-23, 5.1절 측정).** 대상 공고가 넉 달에 2건이다 |
| 공고문 기준비율이 0.90 이 아닐 때 기준비율 입력 | 설계서의 열린 질문 | 사용자 결정 |

### 4.1 `rate_only` 경로 기각 근거

서버의 예정가격 기준액 `prediction_reference_amount` 는 기초금액이 없으면 추정가격으로 대체하므로, 차단은 둘 다 없을 때만 걸린다. 2026-05-26 이후 용역 공고에 실제 판별 함수 `resolve_evaluation_rule_from_raw_data` 와 이 접근자를 그대로 적용해 읽기 전용으로 셌다.

| 범위 | 용역 공고 | 적격심사 판별 성공 | 기초금액 있음 | 추정가격 대체 | 예정가격 없음 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 2026-05-26 이후 | 55,465 | 5,494 | 5,492 | 0 | 2 (0.04%) |
| 2026-09-07 이후 | 6,514 | 668 | 668 | 0 | 0 |

용역 공고의 나머지는 적격심사가 아닌 낙찰방법(25,226), 비예가(19,993), 협상에의한계약(2,573), 수기심사(1,588), 별표 불일치(591)로 먼저 차단된다. 계약 변경과 테스트 기대 반전의 비용에 비해 혜택 공고가 없어 기각했다. 수집 시점과 분석 시점의 차이는 마감 전 공고가 포함된 최근 2주 구간이 0건이라 무시할 만하다.
...
| 3 | 직전 인수인계 4장 3~6번(런북 `MEILI_MASTER_KEY` 한 줄, 코디네이터 작성 브랜치의 게이트 2, restore drill, OP-3) | - | 변동 없음 |
