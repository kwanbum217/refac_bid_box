...
| 순서 | 작업 | 근거 |
| --- | --- | --- |
| 1 | **as-of 집계 표로 평가 시점 고정** | 남은 특징 불일치 8건이 시점 차이임을 **확정했습니다**(학습 대 as-of 상관 0.9999). 코드 결함이 아니라 측정 아티팩트이며, 운영 측정치가 실제를 과소평가하고 있을 수 있습니다 |
...
| 4 | **MLOps 알림 구현** | 설계 완료([`mlops_notification_20260805.md`](../design/mlops_notification_20260805.md)). 주간 크론이 실환경에서 완주한 이력이 없는데 실패해도 통보되지 않습니다. 1시간 규모 |
| 5 | 공고 첨부 문서에서 하한율 확보 | 결측 157,579건. **API 경로는 전부 닫혔습니다.** 파서 구축 비용 판단 필요 |

1번을 먼저 하십시오. 배관 자체는 옳다는 것이 확인됐으므로
([`scripts/verify_asof_history_parity.py`](../../scripts/verify_asof_history_parity.py)),
남은 일은 평가를 예측 시점에 맞추는 것뿐입니다. 그 전까지는 운영 성능을 재도
그 값이 실제인지 아티팩트인지 가릴 수 없고, 그 상태에서 모델을 고치면 오늘처럼
판정이 흔들립니다.
