...
세션 중 다른 에이전트가 같은 저장소에서 작업했습니다. 제 미완성 작업 트리가 `0b781d0` 으로 커밋되고 `e1f7255` 로 병합되는 일이 있었습니다. **중간 상태가 병합됐으므로** 후속 수정 `cbb9e5b` 가 반드시 함께 있어야 정상 동작합니다.

`git add -A` 는 쓰지 않는 편이 안전합니다. 남의 변경을 쓸어 담습니다.
