...
| 순서 | 작업 | 근거 |
| --- | --- | --- |
| 1 | 참가자 수 수집 | 제한경쟁 잔차의 지배 요인. 개찰결과 API 에 존재하나 미수집 |
| 2 | 프론트엔드에 구간 노출 | API 는 준비됐으나 화면이 아직 점 추정만 표시 |
| 3 | 하이퍼파라미터 탐색 | 미수행. 실험본 값에 맞춘 상태이며 추가 여지는 미측정 |

세그먼트별 보정 배율은 **측정 후 기각**했습니다. 전역 배율이 모든 세그먼트에서 88.2~91.1% 로 이미 잘 맞고 세그먼트 배율은 전부 더 나쁩니다. 근거는 설계 문서 7장.
