...
| 순서 | 작업 | 근거 |
| --- | --- | --- |
| 1 | 세그먼트별 보정 배율 | 전역 배율로는 10억 이상 66.85%, 하한율 결측 70.04% 를 못 잡음 |
| 2 | 참가자 수 수집 | 제한경쟁 잔차의 지배 요인. 개찰결과 API 에 존재하나 미수집 |
| 3 | 프론트엔드에 구간 노출 | API 는 준비됐으나 화면이 아직 점 추정만 표시 |
| 4 | 하이퍼파라미터 탐색 | 미수행. 실험본 값에 맞춘 상태이며 추가 여지는 미측정 |
