...
| 순서 | 작업 | 근거 |
| --- | --- | --- |
| 1 | **기각한 세 건 재확인** | 리프 127·255, 지수감쇠. 그 판정들은 **학습 범위가 어긋난 상태**에서 내렸습니다(6.4절). 편향 없는 방식으로 다시 재야 합니다 |
| 2 | 하한율 결측 구간 전용 대책 | 전체의 30%, 오차 3.3배. 오차의 대부분이 여기 있습니다 |
| 3 | 물품 모델을 학습 경로에 올리기 | champion `quantum_leap_v25_pro` 는 특징 4개짜리 **원본 모델**이며 재학습된 적이 없습니다. 용역(35개)과 격차가 큽니다 |
...
1번을 먼저 하십시오. 재적합 수정으로 홀드아웃과 운영의 학습 범위가 처음으로
맞았습니다. 그 전에 내린 기각 판정들은 전제가 달랐으므로 다시 볼 값어치가
있습니다. 편향 없는 비교 틀은
[`scripts/eval_servc_refit_unbiased.py`](../../scripts/eval_servc_refit_unbiased.py)
가 이미 갖추고 있습니다.

3번은 규모가 큽니다. 물품 champion 이 `title`·`agency_name`·`presmpt_prce`·
...
수정 후에는 두 범위가 맞으므로 기각한 세 건을 다시 볼 값어치가 있습니다(6.2 1번).
