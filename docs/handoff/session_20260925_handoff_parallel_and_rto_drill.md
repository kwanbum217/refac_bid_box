...
같은 세션에서 두 웨이브를 더 끝냈습니다. N 웨이브로 **모든 main 병합에 Level 1 strict 증거를 요구하는 훅**(N1)을 넣었고, DB import 1.7배를 환경 1.40배 × 인덱스 1.24배로 나눴습니다. P 웨이브와 확정 실험으로 환경 몫을 Docker VM 저장 계층으로 좁혔고 G1 파일 검증에 세부 계측을 넣었습니다. 세션 끝의 `main` 은 `9e4fc8cf` 이후 이 문서 병합이며 CI 는 Windows 잡까지 성공입니다.
...
| G1 파일 검증이 같은 스냅샷에서 3.2초에서 20.8초로 늘었다 | N4 는 SHA256 해시의 캐시 민감도를 유력 원인으로 추정했으나, P1 계측으로 해시는 합계 약 50MB·0.02초라 틀렸음이 확인됐다(5.1.1 절). 시간은 chroma 구간이다 |
| P1 병합 `8611daaf` 과 뒤이은 문서 병합 `7fa94a42` 의 CI 가 Windows 잡 한 건으로 실패했다. 새 계측이 경로를 `str(path)` 로 기록해 Windows 에서 역슬래시가 들어갔다 | `as_posix()` 로 고쳐 `9e4fc8cf` 에서 Windows 잡까지 성공. 로컬 게이트·리뷰는 macOS 에서만 돌아 OS 의존 경로 결함을 못 잡는다. 경로를 문자열로 남기는 코드는 리뷰 체크리스트에 구분자 항목을 넣는다 |
| 같은 날 같은 스냅샷 2.7% 차이만 보고 "1.7배는 전부 스냅샷 차이" 라고 병합했다가 N3 로 정정했다 | 시간 차이의 원인 판정은 같은 시점에 두 입력을 모두 다시 재서 분리한 뒤 확정한다 |
| 런처 `orca_cmd_launch.py --role` 은 `auto`, `builder`, `reviewer` 만 받는다 | 조사 워커는 `--role auto` 로 띄운다. N4 첫 기동이 `investigator` 로 실패했다 |
...
| Orca | Run `run_073ce2b70842` Task 14건 completed(T1·T2·T4·N1·N2·P1 와 각 리뷰, N4, P2), 워커 터미널 전부 닫음, 배달 큐 비움, 잔류 세션 감사 통과. 터미널 목록의 narani_homepage 창들은 다른 프로젝트라 건드리지 않았다 |
| Docker | 세션 종료 시 `docker compose stop` 으로 전부 내렸다. db 컨테이너는 환경 확정 실험 때 강제 재생성된 것이며 데이터 볼륨은 그대로다. 임시 Meilisearch 컨테이너·볼륨과 drill 격리 DB·임시 디렉터리는 모두 삭제됐다 |
| 배경 프로세스 | 상시 워커 감시기와 측정용 배경 작업 모두 종료. 다음 세션은 Docker Desktop 기동 후 `docker compose up -d db` 부터 시작한다 |
