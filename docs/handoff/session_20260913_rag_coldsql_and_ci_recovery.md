...
| `0810e85b` | 런타임 이미지 gzip·libsqlite3 보안 수정 | 로컬 Trivy 재스캔 통과, CI 확인 대상 |
...
| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | `(dminstt_nm, category, bid_ntce_nm)` 복합 인덱스 채택 여부. 추정 약 760MB, 온라인 DDL 가능, G1 스키마 서명 갱신 필요 (분석 문서 8.2~8.3 절) | 사용자 합의 |
| 2 | 채택 시 Alembic 마이그레이션, 기준선 갱신, 요청 단위 전후 실측 | 1 |
| 3 | ~~2단계 해석 경계 확정~~ 완료. 15만~22만 행, 판정 기준 유지 (분석 문서 8.1 절) | - |
| 4 | ~~벤치마크 하네스 인자 거부~~ 완료 (`930f66a6`) | - |
