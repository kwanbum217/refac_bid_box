...
`main` 은 `795eb49d` 입니다. 이후 낙찰 쪽 커버링 인덱스까지 적용했습니다(6.5 절).
...
| `795eb49d` | 낙찰 기관명 커버링 인덱스와 강제 힌트 (마이그레이션 `c38ebe417cf3`, G1 기준선, 전후 실측) | CI 확인 중 |
...
**운영 반영 시 db 컨테이너 재생성이 필요합니다.**

### 6.7 확인하지 못한 것

- `128e93fe`, `9a829fea` CI 는 이후 전 잡 성공을 확인했습니다.

## 7. 다음 착수 순서

| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | `795eb49d` CI 결과 확인 | 없음 |
...
| 원격 | `origin/main` = `795eb49d` (이 문서 병합 전) |
