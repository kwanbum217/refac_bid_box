...
| `128e93fe` | 부분 일치 되돌림 + category 공고 집계 커버링 인덱스 강제 힌트 | CI 전 잡 성공 |
| `9a829fea` / `4804f18c` | 원장 `coldsql_rerun` 종결, 고아 FTS 사전 항목 복제본 검증 기록 | CI 전 잡 성공 |
...
**힌트 병합 후 정본 재측정은 메모리 부족으로 세 번 강제 종료돼 하지 못했습니다(6.4 절).**
...
**사용자 허용 후 복제본에서 검증했습니다(분석 문서 11.1.1 절).** 테이블 재구축은 65초에 오류 없이 끝나고 데이터 지문이
원본과 같았지만 고아 항목 6개는 남았고, 직접 `DROP TABLE` 은 `ERROR 1146` 이었습니다. 남은 제거 방법은 논리 덤프를 새 인스턴스에
복원하는 것뿐입니다. 정기 백업이 논리 덤프라 백업 위험은 낮고, **재구축이 성공했으므로 `bid_results` DDL 은 이 항목에 막히지
않습니다.** 개발 DB 에는 아무것도 적용하지 않았습니다.
...
### 6.5 낙찰 기관명 커버링 인덱스 (사용자 권고안 위임, 분석 문서 14장)

복제본 검증(6.3 절)으로 고아 항목이 DDL 을 막지 않음을 확인한 뒤 `ix_bid_results_inst_cat_stats
(dminstt_nm, category, bidwinnr_nm, sucsf_bid_rate, sucsf_bid_amt)` 를 넣었습니다(17초, 396MB, G1 6항목 통과).
category 가 없으면 옵티마이저가 스스로 이 인덱스를 고르고, "광주"+Servc 는 category 인덱스를 골라 같은 조건의 강제 힌트를
더했습니다. **SQLAlchemy `with_hint` 는 테이블마다 힌트 하나라 낙찰업체 집계에서는 이 강제가 그룹 인덱스 배제를 대체합니다.**

| 구간 | q08 전 → 후 | q03 전 → 후 |
| --- | --- | --- |
| 낙찰 금액 집계 | 1.2~1.4초 → **0.47~0.53초** | 1.7~2.1초 → **0.8~1.3초** |
| 낙찰업체 집계 | 1.2초대 → **0.66~0.70초** | 0.9초대 → **0.64~0.67초** |
| 요청 전체 | 8.3~9.2초 → **5.7~6.4초** | 8.5~19.3초 → 6.9~12.5초 (해석 구간 튄 값으로 중첩) |

**운영 DB 반영 시** `alembic upgrade head` 로 `afc72b545c6a`, `c38ebe417cf3` 두 리비전이 차례로 들어갑니다.

### 6.6 MySQL 임시 테이블 상한 128MB (분석 문서 16장)

q08 공고명별 집계가 16MB 상한을 넘어 매회 디스크 임시 테이블로 넘어가 2.5초가 걸렸습니다. 32·64MB 는 여전히 디스크였고
128MB 에서 전환이 사라져 1.7초가 됐습니다. 두 compose 의 db `command` 에 반영했고 개발 DB 는 재생성해 적용을 확인했습니다.
**운영 반영 시 db 컨테이너 재생성이 필요합니다.** q08 요청 전체 시간은 다시 재지 않았습니다.

병합 첫 시도는 병합 전 전량 테스트 증거가 이전 커밋(`ba1a4d06`) 기준이라 훅이 거부했습니다. 브랜치 HEAD 에서
`premerge_full_suite_gate.py --record` 로 다시 기록한 뒤 병합했습니다. **설정·문서 브랜치도 병합 직전에 증거를 새로 기록해야 합니다.**
...
| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | 운영 반영: `alembic upgrade head`(`afc72b545c6a`, `c38ebe417cf3`)와 db 컨테이너 재생성(임시 테이블 상한) | 운영 배포 시점 |
| 2 | 고아 FULLTEXT 사전 항목 제거. 복제본 검증 완료, 남은 방법은 논리 덤프 복원 (6.3 절) | 컷오버·업그레이드 시점에 묶기 (권고안 채택) |
| 3 | ~~낙찰 금액 집계 커버링 인덱스~~ 적용 완료 (6.5 절) | - |
| 4 | 힌트 병합 후 전체 fixture 정본 재측정 | 메모리 (측정 세 번 강제 종료) |
