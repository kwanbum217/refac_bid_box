...
### 6.7 128MB 요청 단위 재측정과 기관명 목록 캐시 (분석 문서 16.1 절, 17장)

- 128MB 적용 후 q08 요청 전체가 5.7~6.4초에서 **5.1~5.7초**로 줄었습니다(완전 콜드 2라운드).
- q03 의 튄 값은 1라운드 공고 기관명 해석("광주") 12.9초였습니다. 상한 안 검색어는 기관명 인덱스를 끝까지 훑습니다.
- 한글 음절 전용 검색어는 고유 기관명 목록(`rag:inst_catalog:<테이블>`, 1시간)에서 파이썬으로 해석하게 했습니다.
  괄호·영문·숫자·공백이 섞이면 종전 해석 질의입니다. 파이썬 비교가 LIKE 와 어긋나는 문자 종류를 실측으로 좁혔습니다.
- 전후: q03 1라운드 20.7초 → **12.6초**, q08 5.1~5.7초 → **4.9~5.0초**, 목록을 가진 요청의 해석 구간 약 40ms.
- 목록을 만드는 요청은 콜드에서 여전히 인덱스를 읽습니다. 야간 예열이 후속 후보입니다.

### 6.8 독립 집계 병렬 실행과 기관명 목록 매시 예열 (분석 문서 18장)

- 정형 검색 집계 다섯 개를 MySQL 에서 병렬 실행합니다(프로세스당 스레드 4개). q08 요청 전체 4.9초 → **2.1~2.3초**,
  q03 2라운드 6.9초 → **3.6초**. 문장마다는 경합으로 10~30% 느려집니다.
- arq `refresh_institution_catalog_task` 가 매시 5분 기관명 목록을 예열합니다. 개발 환경에는 워커가 없어 예열 효과는 실측하지 않았습니다.
- 운영 반영 시 앱 재기동과 arq 워커 재기동이 필요합니다.
- 후속 확인(분석 18.1 절): q25·q31 병렬 회귀 없음(q31 0.17~0.21초), 동시 1~4건 전 지표 개선·8건은 중앙값 0.8초 악화와 꼬리 개선, 연결 최대 12개. 예열 작업 실행 757ms, Redis TTL 7,200초 확인.

### 6.9 Windows CI 연결 대기 제거 (세션 종료 후 추가 작업)

Grok 의 CI 대기 단축 제안서(`~/Downloads/refac_bid_box_ci_wait_reduction_proposal_20260913.md`)를 검토했습니다. 권고안 B(경로별
잡 분리)는 원격에 `main` 만 푸시하는 이 저장소에서 효과가 거의 없고 문서를 읽는 테스트 24개 파일을 놓칠 위험이 있어 보류했습니다.
대신 병목인 Windows pytest 를 조사했습니다.

- 원인: Windows 는 닫힌 localhost 포트 연결이 4.33초 뒤 실패합니다. 세션 저장소·로그인 제한 Redis 연결이 5초 백오프마다 다시
  시도해 로그인 테스트마다 기다렸습니다. `0d3ea959` 의 PBKDF2 가속 뒤에도 남아 있던 균일 지연입니다.
- 재현: 로컬에서 같은 지연을 흉내 내면 전량 테스트 90초 → 240초, 느린 테스트 목록이 CI 와 일치했습니다.
- 수정(`2dba1f82`): `tests/conftest.py` 의 `_fast_fail_local_services` 가 로컬 Redis(동기·비동기)와 Ollama 포트 연결을 즉시
  실패시킵니다. 운영 코드는 바꾸지 않았습니다. 흉내 조건 240초 → 88.6초.

| CI (Run) | 전체 | Windows 잡 | Windows pytest | Ubuntu 3.11 pytest |
| --- | --- | --- | --- | --- |
| 전 (34760809083) | 9분 4초 | 8분 57초 | 7분 52초 | 2분 24초 |
| 후 (34762922494) | **5분 37초** | **5분 33초** | **4분 39초** | 2분 23초 |

남은 4.2초 대기 3건(`test_e2e_full_flow`, `test_chatbot_stream_canonical`, MySQL 통합 setup)은 합계 약 13초입니다. MySQL 은 CI 통합
...
| 4 | ~~힌트 병합 후 전체 fixture 정본 재측정~~ 2026-09-14 완료. canonical 통과, 콜드 SQL P95 15.1초 → 1.9초, 최대 82.5초 → 5.1초 (분석 문서 19장). `nohup` 분리 프로세스로 완주 | - |
