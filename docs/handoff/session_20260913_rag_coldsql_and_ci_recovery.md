...
1초대로 줄였고, 병렬 워커로 `source_commit` 뒤처짐을 병합 단위로 세게 했습니다. `main` 은 `f1dad950` 이며
CI 10개 잡이 전부 성공입니다.
...
| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | `bid_results` 기관명 조건 집계(q08 잔여 약 30초) 원인 귀속과 대책. 금액·낙찰자까지 담는 커버링 인덱스는 크기 추정부터 | 없음 (DDL 은 합의) |
| 2 | 원장 `missing_lwlt_intervals` 정리 (구현 완료 `77773ae2`, CURRENT_STATE active 목록과 함께) | 없음 |
| 3 | 웜 회차 최대값 증가(25~32ms → 62~63ms) 재현 확인 | 없음 |
| 4 | 운영 DB 에 `afc72b545c6a` 적용 | 운영 반영 시점 |
...
| 원격 | `origin/main` = `f1dad950`, CI 전 잡 성공 |
