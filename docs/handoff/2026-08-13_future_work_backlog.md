...
| --- | --- | --- | --- |
| P0 | Servc 수집 복구와 누락일 회수 | 수집 사실상 중단 | 실행일까지 백필하고 중복 없이 최신일 전진 |
...
현재 개발 수집 창은 전일 하루뿐입니다(`src/app/services/collector_service.py:77`).
프로세스가 여러 날 중단되면 그 기간이 영구 공백이 됩니다.

권장안은 스키마 변경 없이 시작할 수 있는 **제한된 중첩 수집 창과 명시적 백필
체크포인트**입니다. 구현 전에 다음을 결정합니다.

1. 자동 중첩 기간과 최대 회수 기간
2. 외부 API 호출량·속도 제한
3. 부분 실패 뒤 재개 위치와 실패 가시성
4. 공고·결과 각각의 멱등성 기준
5. 하루 중단, 나흘 중단, 부분 실패, 동일 구간 재실행 테스트

고수위 표식 저장이 필요해 스키마 변경이 발생한다면 데이터 보존 검증과 사용자
승인을 별도 게이트로 둡니다.
