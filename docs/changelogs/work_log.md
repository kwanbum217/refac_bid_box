...
- **원인 제거**: `tune_servc_hyperparams.py` 의 `evaluate` 를 운영 `train_and_register` 3단계로 맞췄습니다. 학습 구간을 `openg_dt` 시간순 80/20 으로 가르고 뒤 20%를 `eval_set` 으로 조기 종료해 트리 수를 정한 뒤 그 트리 수로 학습 구간 전량을 재적합합니다. 검증 연도를 `eval_set` 으로 쓰면 트리 수를 검증 연도로 정하고 같은 연도에서 점수를 내게 되어 누수이므로 분할은 학습 구간 안에서만 합니다. 학습이 2회로 늘어도 시행 시간은 25% 증가에 그칩니다(실측 39.5초 -> 49.4초). 조기 종료가 첫 학습을 일찍 끊어 재적합 비용을 상쇄하기 때문입니다. 기준선 MAE 는 1.2554 대 1.2557 로 거의 같은데 현행 설정에서 조기 종료가 상한 근처에서 걸리기 때문이며, 경로 차이는 학습률을 낮춰 트리 수로 보상하려 할 때 드러납니다. 이 수정 이전 탐색 결과의 절대값은 새 경로와 비교할 수 없습니다

---

### 2026-08-07 | 용역 준비 | 잔차 parquet 에 식별자 추가

- **작업자**: 관범 & Claude
- **배경**: 기관 x 소분류 교차 이력(다음 축 2순위)의 사전 신호를 잔차 parquet 으로 재려 했습니다. 특징을 만들어 재학습하면 40분이 넘지만 잔차로는 몇 분이면 값어치를 판단할 수 있습니다
- **막힌 지점**: 저장된 잔차에 기관 식별자가 없었습니다. `dminstt_nm` 은 원본 parquet 에만 있는데, 행 순서로 맞추려 해도 같은 `openg_dt` 안의 순서가 달라 2025년 96,141행에서 `clsfc_nm` 일치율이 12% 였습니다. 안전한 조인이 불가능합니다
- **되돌릴 수 없었던 이유**: 잔차 재생성에 35분이 걸립니다. 남은 시간에 재생성하고 분석까지 마칠 수 없어 분석 자체는 다음 세션으로 넘겼습니다
- **조치**: `diagnose_servc_lwlt_residuals.py` 가 `bid_ntce_no` / `dminstt_nm` / `ntce_instt_nm` 을 함께 남깁니다. 진단 축으로 붙였으므로 학습에는 들어가지 않습니다(`fit_until` 이 `feature_columns` 만 골라 씁니다). 기존 4개 연도 parquet 에는 없으므로 이 분석 전에 재생성이 필요합니다
- **역설**: 해당 코드에는 "컬럼을 골라 남기지 않습니다. 나중에 필요한 축이 빠져 있으면 그 35분을 다시 써야 합니다" 라는 주석이 이미 있었습니다. 특징 프레임만 남기고 원본 식별자를 빠뜨려 그 의도가 실패했습니다
- **인수인계**: 4.1 절에 사전 신호 측정 순서(오라클을 재현성보다 먼저)를, 4.2 절에 이 벽과 재생성 명령을 적었습니다

---

### 2026-08-07 | 용역 측정 | LightGBM + CatBoost 앙상블 여지 확인

- **작업자**: 관범 & Claude
- **배경**: `train_and_register` 는 MAPE 최저 하나만 고릅니다. 앙상블을 넣으려면 선택 로직을 손봐야 해 범위가 크므로, 구현 전에 여지부터 쟀습니다
- **판정 기준 사전 고정**: 잔차 상관 0.95 이상이면 같은 곳에서 틀리는 것이므로 닫고, 상관이 낮아도 반반 평균이 단일 최고를 못 이기면 여지가 작다고 봅니다. 가중치를 최적화하기 전의 가장 무딘 형태조차 이득이 없으면 최적화해도 이득이 작습니다
- **측정**: `scripts/eval_servc_ensemble_headroom.py` 신설. 2025년 96,141건, 2024년까지 학습, 운영 카테고리 설정 그대로입니다. 조기 종료는 학습 구간 내부 80/20 으로 걸어 검증 연도 누수를 피했습니다
- **결과**: LightGBM MAE 1.3593 / 0.5%p 59.93%, CatBoost 1.5835 / 44.34%, 반반 평균 1.4404 / 54.50% 입니다. 잔차 상관은 0.9389 로 한계 아래인데도 반반 평균이 LightGBM 대비 MAE +0.0811 나쁩니다
- **구조적 원인**: 반반 평균은 RMSE 2.7351 과 R2 0.6757 로 둘 다를 이기고 운영 지표인 MAE 와 0.5%p 적중에서는 집니다. LightGBM 은 quantile(0.5)로 조건부 중앙값을, CatBoost 는 기본 RMSE 로 조건부 평균을 겨냥합니다. 평균을 내면 중앙값 겨냥이 희석되고 용역 잔차는 0 에 몰린 비대칭이라 그것이 곧 MAE 손해입니다
- **R2 함정 세 번째**: "CatBoost 가 R2 0.6994 로 LightGBM 0.6967 을 이겼다" 는 과거 관찰이 앙상블 축의 근거였는데 R2 함정의 재현이었습니다. 잔차가 비대칭이면 중심을 위로 민 모델이 R2 에서 유리합니다
- **강등**: 기각이 아니라 강등입니다. CatBoost 에 L1 계열 목적함수를 주고 다시 재는 갈래는 미측정입니다. 다만 상관 0.9389 가 한계에 가깝고 목적함수를 맞추면 상관은 더 오를 가능성이 커 우선순위는 낮습니다
- **인수인계**: 3장을 측정 결과로 교체하고 순위를 재배열했습니다. 남은 1순위는 기관 x 소분류 교차 이력 하나입니다

---

### 2026-08-09 | 용역 측정 | 기관 x 소분류 교차 이력 기각

- **작업자**: 관범 & Claude
- **배경**: `quantile(0.5)` 승격 이후 남은 마지막 사전 순위 축이었습니다. 앙상블은 2026-08-07 에 강등됐고 교차 이력만 남아 있었습니다
- **측정 설계**: `scripts/eval_servc_inst_clsfc_cross.py` 신설. 특징을 만들어 재학습하면 40분이 넘으므로 잔차 parquet 4개 연도 346,936행에서 사전 신호를 쟀습니다. 스킬 원칙대로 재현성보다 오라클을 먼저 두었고, 판정 기준을 결과를 보기 전에 최소 표본 20 / 중앙값 오프셋으로 고정했습니다
- **오프셋 통계량 교정**: 운영 지표가 MAE 이고 목적함수가 `quantile(0.5)` 이므로 MAE 를 최소화하는 상수 오프셋은 셀 잔차의 중앙값입니다. 앞선 기각들이 쓴 평균 오프셋은 이 점에서 불리했으므로 두 통계량을 함께 쟀습니다
- **오라클은 처음 통과했습니다**: 네 해 모두 양수(2023 +0.0384, 2024 +0.0320, 2025 +0.0313, 2026 +0.0247). 하한율·낙찰방법·소분류 세 축이 오라클에서 이미 음수였던 것과 다릅니다. 교차 셀은 잔차를 실제로 가릅니다
- **실용에서 전부 음수**: 전년 오프셋을 다음 해에 적용한 12개 조합이 전부 음수입니다(-0.0013 ~ -0.0188). 임계값을 올리면 손해가 줄지만 그것은 개선이 아니라 적용 범위가 2% 대로 좁아지는 것입니다
- **교차라는 형태 자체의 기각**: 같은 조건에서 기관 단독 오라클 +0.0558, 교차 +0.0316 입니다. 셀을 잘게 쪼개면 임계값을 넘기는 행이 줄어 적용 범위가 좁아지고 늘어난 해상도가 그 손실을 메우지 못합니다. 2026년 56,338행이 셀 23,761개로 나뉘고 20건 이상은 285개(행 비중 17.5%)뿐입니다
- **특징 형태도 같은 벽**: `features.py` 가 만들 수 있는 교차 이력은 서빙 시점에 과거만 쓰므로 실용 단계와 동형입니다. `features.py` 는 손대지 않았고 재학습·승격도 없습니다. 서빙은 `v_20260807_043210_535` 그대로입니다
- **부수 확인**: 중앙값 오프셋이 평균을 모든 조합에서 이겼고, 소분류 단독은 통계량을 바꾸자 오라클 부호가 -0.1525 에서 +0.0083 으로 뒤집혔습니다. 그런데도 실용 결론은 -0.0013 으로 그대로였습니다. 통계량 선택이 문제가 아니라 셀 오프셋이 그 해의 사정이지 구조가 아닌 것이 문제입니다
...
### 2026-08-09 | 용역 측정 | CatBoost L1 목적함수 기각, 앙상블 축 종결

- **작업자**: 관범 & Claude
- **배경**: 2026-08-07 반반 앙상블 기각 때 "목적함수가 L1 대 L2 라 평균이 중앙값 겨냥을 희석한다" 로 원인을 읽고, CatBoost 에 L1 을 주는 갈래를 미측정으로 남겼습니다. 사전 순위 축 중 마지막이었습니다
- **측정**: `eval_servc_ensemble_headroom.py` 에 `--catboost-loss` 옵션 추가. 나머지는 2026-08-07 과 동일하게 2024년까지 학습(765,150행), 2025년 검증(96,141행), 조기 종료는 학습 구간 내부 80/20
- **진단은 맞았습니다**: CatBoost 단독 MAE 1.5835 -> 1.4809, 0.5%p 적중 44.34% -> 59.73% 로 LightGBM 적중률 59.93% 에 거의 닿았습니다. 반반 평균 손해도 -0.0811 -> -0.0397 로 절반이 됐습니다
- **그런데 여지는 없습니다**: 가중치를 0.00~1.00 까지 0.05 간격으로 훑자 MAE 가 단조 감소하고 최적이 LightGBM 100% 였습니다. 가중치를 검증 연도에서 고르는 오라클 상한조차 단일 최고 대비 +0.0000 입니다. 어떤 비율로 섞어도 손해입니다
- **구조적 이유**: 잔차 상관 0.9322 에서 독립 오차 성분이 작은데 CatBoost 가 MAE 에서 0.12 열등합니다. 거의 같은 방향으로 틀리는 열등한 모델을 섞으면 열등함만 옮겨옵니다
- **예상이 틀린 지점**: 목적함수를 맞추면 두 모델이 닮아 상관이 오를 것으로 봤는데 0.9389 -> 0.9322 로 미세하게 내렸습니다. 결론에는 영향이 없습니다
...
### 2026-08-09 | 개발 데이터 최신화 | 운영·개발 스케줄 분리 및 수동 수집 검증

- **작업자**: 관범 & Codex
- **변경**: 기본 Compose worker에 `AUTOMATION_DATA_REFRESH_SCHEDULE_ENABLED=true`를 추가하고, 매일 02:00 KST에 `collect → rag → inspect`와 순위·기관 집계를 실행하도록 분리했습니다. 운영 야간 번들 및 주간 재학습은 기본 비활성화이며, 둘을 함께 켜면 개발 작업은 중복 수집을 막기 위해 건너뜁니다
- **수집 키 정합성**: Compose가 서비스 키를 worker와 app에 전달하고, 표준 `G2B_SERVICE_KEY`와 기존 `serviceKey`를 모두 지원하도록 수집기를 보완했습니다
