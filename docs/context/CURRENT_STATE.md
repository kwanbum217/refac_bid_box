...
- **compare-stats 병목 규명 (2026-09-10, 조사 완료·미구현)**: 캐시 미적중 6.5초의 최우선 병목은 매칭 건수 쿼리입니다. `dashboard.py:503` 의 `IN` 서브쿼리가 결과 기간 조건이 있는데도 `bid_results` 약 311만 행을 전부 읽습니다(`ALL`, 인덱스 미사용, `Start temporary`). 의미를 보존한 `EXISTS` 대안은 `rl_openg_dt` 범위 접근으로 예상 행을 약 72만으로 낮췄습니다. 나머지 셋(기관별 상위 10, 월별 집계 두 회)은 `Using temporary; Using filesort` 이며 예상 행이 74만에서 104만입니다. 방안별 기대 효과와 위험은 `docs/analysis/at4_compare_stats_optimization_20260910.md` 에 있습니다. **순위는 `EXPLAIN` 기반 추정이며 승격 전에는 실행시간·논리 읽기·결과 동등성을 별도로 검증해야 합니다.** 단순 `JOIN` 은 중복 때문에 매칭 건수를 바꾸므로 `EXISTS` 나 `COUNT(DISTINCT)` 를 써야 합니다. 구현은 아직입니다.
