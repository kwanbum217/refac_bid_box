...
| # | 금지 항목 | 결론 |
| :---: | --- | --- |
| 1 | Uvicorn 워커 증설 (3/4) | 개선 없이 코어 경합만 증가. 기각 |
| 2 | `PREDICTION_GC_MODE=batch-disable` | 100ms 초과 30건으로 기본(21)보다 악화. 기각 |
| 3 | `PREDICTION_GC_MODE=threshold` | `freeze` 확정으로 상호 배타. 재검토 불필요 |
| 4 | `--inject` 도달 미확인 | 승인 전후 모두 유실 사례. 도달은 `terminal read` 로만 확인 |
| 5 | 무료 LLM 풀의 임계 경로 투입 | 자동 선택 대상 아님. `investigator`+`low`+쓰기 범위 없음에서만 `--allow-free`. 실패로 단정하지도 않음 |
| 6 | 미병합 브랜치 재병합 | `codex-task-routing`·`arq-worker-cutover` 폐기, `perf/predict-tail` 진단 전용 |
| 7 | 워커 요약 텍스트만 신뢰 | diff 와 결정론 검증을 직접 대조 |
...
| 11 | 크기 예산을 바이트로 판정 | 8,000자는 문자 수다. `wc -c` 금지, `len()` 사용 |
...
| 16 | 코디네이터 검토를 Level 2 대체로 사용 | 코디네이터 검토만으로 병합한 도구 4개에서 독립 감사가 결함 7건을 찾았다. 두 층을 다 통과시킨다 |
| 17 | 미확인 외부 CLI 서명으로 코드 작성 | 값 유무는 `--help` Usage 줄로 본다. `opencode ask` 는 실패하며 종료 코드 0 을 냈다 |
| 18 | 테스트가 틀린 사실을 정답으로 고정 | 없는 CLI 서명을 기대값으로 단정했다. 통과하는 테스트는 확인의 근거가 아니다 |
| 19 | `defect_when` 에 산문 기재 | `yes`/`no` 극성 토큰이다. 문장을 넣어 8항목이 무효화됐다 |
| 20 | `ready` 복귀 후 재 Dispatch 생략 | 권한 회수로 재보고가 거부된다. 병합한 Task 는 `completed` 로 닫는다 |
| 21 | `worker-list` 로 동시 워커 수 판정 | `worker-start` 워커만 보여 3대가 붙어도 활성 0. `task-list` 의 `dispatched` 로 판정한다 |
| 22 | 미실측 모델 ID 등록 | 선택되는 순간에야 `Model not found`. 추가 시 그 자리에서 probe 한다 |
| 23 | 읽기 범위를 차단 장치로 취급 | 쓰기 범위는 `git diff` 검증, 읽기 범위는 자기 신고만 대조. 유출 금지 대상은 워커 트리에 두지 않는다 |
...
| 26 | Capsule 경로 미주입 | `--inject` 는 Task `spec` 만 보낸다. 워커 3/3 이 위반했다. 경로를 `spec` 에 넣고 새 `dispatchId` 도 전달한다 |
| 27 | `check` 를 `--ack` 없이 호출 | 미확인 배치가 재전달되어 뒤의 `worker_done` 이 가려지고 `--wait` 도 즉시 반환된다 |
| 28 | 동작 보존 분할을 사람 판독으로만 검증 | `ast.dump(node, include_attributes=False)` 로 기계 대조한다. 리뷰어가 놓친 본문 변경 5건을 찾았다 |
| 29 | 워커 린터 보고를 전체 통과로 읽기 | 워커는 지정 경로만 검사한다. 게이트 4b 가 저장소 전체 `ruff check .` 를 돌린다 |
| 30 | AST 대조 이름을 평평하게 모으기 | 메서드와 모듈 함수가 같은 이름일 수 있어 거짓 양성이 난다. `Class.method` / `<module>.func` 로 정규화한다 |
| 31 | 저장소 도구를 `python3` 로 실행 | macOS `python3` 는 Xcode 3.9 라 `datetime.UTC` 가 깨진다. `requires-python` 이 `>=3.11` 이니 `uv run python` 을 쓴다 |
| 32 | 계약 이름만 적고 필드명 미열거 | `return_contract` 는 라벨이라 구속력이 없다. 워커 2/4 가 필드명을 바꿔 냈다. `report_schema` 로 열거한다 |
| 33 | TPM 을 파일 수 축소로 해소 | 원인은 루프의 컨텍스트 재전송이다. 파일 1개도 실패, 사실 주입 원샷만 통과 |
| 34 | 한 Run 의 순차 행을 모델 비교로 읽기 | 뒤 행일수록 캐시가 서 있어 순서 효과와 모델 효과가 섞인다 |
...
1. **Orca 코디네이터 토큰 최적화 v2 — 구현 완료, 실사용 교정 중**:
   - **수신면 도구 5종**: `orca_contract.py`, `orca_level1_gate.py`(게이트 6대), `summarize_worker_done.py`, `orca_run_reviewer.py`, `orca_metrics_ledger.py`. 독립 감사 결함 7건 수정.
   - **Level 3 유지 확정**: 결함 밀도 0.05 사전 등록, A/B 모두 검출 0/3.
   - **Control Plane 도구**: `orca_taskctl.py`, `orca_model_router.py`. `3453a3f` 를 되돌린 사유가 3장 17~20번입니다.
   - **동시 쓰기 워커 상한 3대**를 `dispatch` 가 기계 강제(AGENTS.md 4장 5.1).
   - **2026-08-17 실사용 도구 결함 4건 수정**: 기동 경로(`76c3013`), Capsule 경로 미주입(`eb6f429`), 중복 생성(`cef6623`), 템플릿 산출물 경로(`b86d833`). 넷 다 문서상 완비였고 실사용에서만 드러났습니다.
   - **절감량**: 도입 전 값은 확보 불가 유지. 2026-08-17 순차 단독 Dispatch 4건으로 대표 지표 유효 행을 0 -> 4 로 확보(중앙값 6,412). 다만 순서 효과와 모델 효과가 섞여 추세 판정은 아직 불가(3장 34번).
   - **2026-08-17 모델별 동형 감사 4종**(gemini-flash-high, sonnet-4-6, opus-4-6-thinking, cerebras/gemma-4-31b): 읽기 전용 4건 전부 저장소 무수정. 도구 결함 3건 추가 수정(`read_scope`, 리뷰어 쓰기 범위 비움, `report_schema`).
2. **대형 모듈 분할 (Phase 3) — 완료**: 5개 모듈 5,061 -> 2,617줄(-48%). 1,000줄 초과 0개, 신규 13모듈 최대 514줄. AST 동일성으로 동작 보존 실증(1차 65개, 2차 85개). 다음 기준선 600줄 후보 판정은 2026-08-17 감사 4건 참조.
