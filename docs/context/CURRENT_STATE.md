...
1. **Orca 코디네이터 토큰 최적화 v2 — 구현 완료, 실사용 교정 중**:
   ...
   - **절감량**: 도입 전 값은 확보 불가 유지. 2026-08-17 순차 단독 Dispatch 로 대표 지표 유효 행 0 -> 5 확보(중앙값 7,363). 순서 효과와 모델 효과가 섞여 추세 판정은 아직 불가(3장 34번).
...
2. **대형 모듈 분할 — 1,000줄 완료, 600줄 진행**: 6개 모듈 5,899 -> 3,119줄, 신규 16모듈 최대 514줄. AST 동일성 실증(65+85+24). `trainer.py` 838->502 는 감사 경계를 그대로 실행한 첫 건. 600줄 초과 잔여는 `planner.py` 643 하나이며 감사가 분할 권고했다.
