...
| 16 | 코디네이터 검토를 Level 2 대체로 사용 | 10번의 역방향. 코디네이터 검토만으로 병합한 도구 4개에서 독립 감사가 결함 7건을 찾았다. 두 층을 다 통과시킨다 |
...
| 31 | 저장소 도구를 `python3` 로 실행 | macOS `python3` 가 Xcode 3.9 라 `datetime.UTC` 임포트가 깨진다. `requires-python` 이 `>=3.11` 이므로 코드가 맞다. `uv run python` 을 쓴다 |
...
1. **Orca 코디네이터 토큰 최적화 v2 — 구현 완료, 실사용 교정 중**:
   - **수신면 도구 5종**: `orca_contract.py`, `orca_level1_gate.py`(Level 1, 게이트 6대), `summarize_worker_done.py`, `orca_run_reviewer.py`(Level 2), `orca_metrics_ledger.py`. 독립 감사 결함 7건 수정.
   ...
   - **2026-08-17 실사용에서 도구 결함 4건 수정**: `dispatch` 기동 경로(`76c3013`), Capsule 경로 미주입(`eb6f429`, `create` 신설), Capsule 중복 생성(`cef6623`), 템플릿 산출물 경로(`b86d833`). 넷 다 문서상으로는 완비돼 있었고 실사용에서만 드러났습니다.
   - **절감량**: 도입 전 값은 확보 불가 유지. 추세는 원장 행이 더 필요합니다.
2. **대형 모듈 분할 (Phase 3) — 완료**: 5개 모듈 5,061 -> 2,617줄(-48%). `engine.py` 1171->490, `automation_orchestrator.py` 1039->550, `planner.py` 968->643, `model_registry.py` 1091->400, `chatbot.py` 792->534. 1,000줄 초과 0개, 신규 13모듈 최대 514줄. AST 동일성으로 동작 보존 실증(1차 65개, 2차 85개).
