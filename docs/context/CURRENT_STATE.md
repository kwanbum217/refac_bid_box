...
- **적격심사 정량평가 (2026-09-09, 동작)**: 공고 상세 페이지에 공고 기준 분석 카드를 붙였습니다. 판별 정본은 `raw_data.sucsfbidMthdNm` 문자열이고 별표 14종 하한율은 DB 550만 건 실측으로 확정했습니다(보험용역은 문서 47.994 대 실측 **47.995**). 가격배점 B 와 계수 k 와 통과점수 T 는 별표마다 다르고 공고 데이터에 없어 **사용자가 공고문 배점표를 입력**하며, 셋 중 하나라도 없으면 `MISSING_SCORE_TABLE` 로 점수만 차단하고 하한율 기반 최저 투찰금액과 예정가격 시나리오는 계속 제공합니다. 실사용 검증은 별표 14종 각각에 실재 공고를 붙여 `tests/test_evaluations_realuse_scenarios.py` 로 고정했습니다. 남은 것은 기술용역 적격심사이며, 44개 식별 문자열을 공통 배점표로 환원할 규칙이 없다는 것이 조사로 확인됐습니다.
