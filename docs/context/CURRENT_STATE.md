...
| **G2** | 크로스 플랫폼 | **보류 (Windows 조사 중)** | MySQL ngram job 은 **green 복귀**했고 Ubuntu 3종·macOS·Docker·lint 도 green 입니다. **Windows job 만 실패**하며 네 층을 닫아 342초를 212.90초로 줄였으나 잔여 원인이 남아 2026-09-01 에 `continue-on-error` 로 **게이트에서 제외**했습니다(job 은 계속 돌려 관측 유지, [`windows_ci_soft_fail_20260901.md`](../analysis/windows_ci_soft_fail_20260901.md)). **게이트 제외를 통과로 올리지 않습니다.** Windows Docker Desktop 실기 미수행은 여전히 별개 항목입니다 |
...
> **2026-09-01 18:58 재평가**: 외부 감사가 지적한 대로 **현 HEAD 의 CI 가 RED** 입니다.
> 컷오버 선언 자체를 철회하지는 않되(G1 무손실과 G3 레이턴시의 실측 근거는 유효),
> **현 HEAD 를 릴리스 정본으로 취급하지 마십시오.** Windows CI 회귀와 ngram 통합
> CI 계약 모순을 닫고 전체 green 을 회복한 뒤에 다시 판정합니다. 진행 상황은
> [`../ops/handoff_20260901_cutover.md`](../ops/handoff_20260901_cutover.md) 4 장입니다.
