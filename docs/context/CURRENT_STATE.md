...
- **채점 규약 결함 (미해결)**: fixture 의 `context_sufficient` 는 검색 성공을 전제하므로, 검색이 기대 문서를 못 가져온 상태의 정직한 거부가 과잉응답으로, 거부 답변의 인용 부재가 citation 누락으로 집계됩니다. **환각을 보상하고 정직을 벌하는 집계**입니다. 2026-08-30 측정에서 q21 이 여기 해당하며(이전에는 다른 지구 공고를 사실처럼 답한 환각, 이번에는 정직한 거부), 이 때문에 citation 이 100% 에서 97.2% 로 보입니다. `evidence_recall == 0` 인 요청을 두 집계에서 분리해야 합니다.
- **q21 검색 실패 (미해결, T7 과 무관)**: "2026년 조림지 풀베기사업 2차(동부지구)" 질의가 기대 근거 `bid_10169448` 을 lexical·vector 어느 경로에서도 후보에 올리지 못합니다. 두 측정 모두 recall 0.0 입니다.
