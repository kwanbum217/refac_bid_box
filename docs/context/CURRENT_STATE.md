...
- **KB 색인 메모리 폭주 (2026-09-08, 수정 미검증)**: `rebuild_knowledge_base` 가 최근 1년치 문서를 전량 적재해 worker 컨테이너가 10.5GB 를 쓰고 소멸합니다. 로그의 `disk I/O error` 는 증상이며 원인이 아닙니다. Docker 메모리 상향으로는 해결되지 않습니다(14->16GiB 에서도 재현). 청크 스트리밍 수정이 `kwanbum217/wave-ap1` @ `9293642` 에 있으나 **검증 미실행·미병합**입니다.
