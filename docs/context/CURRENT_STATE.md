...
- **observability**: 관측성 스택은 Collector·Tempo·Grafana 로 확정했고 요청 지연·DB 질의 지연·요청 수 메트릭 계측은 병합했습니다. 2단계 Prometheus 배선을 추진합니다.
...
- **관측성 2단계 Prometheus (2026-09-11, 계측 완료·배선 미착수)**: 1단계 Collector·Tempo·Grafana 배선은 `docker-compose.prod.yml` 에 들어갔고, OTel 메트릭 계측(요청 지연·DB 질의 지연·요청 수, 템플릿 라벨, `OTEL_ENABLED` 꺼짐 시 무자원)은 `8a1f8fa1` 에 병합했습니다. Prometheus 서비스 배선과 Collector 메트릭 exporter 가 남았습니다.
