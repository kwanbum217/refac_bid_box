...
- **servc_qualification_evaluation**: 일반용역 적격심사 정량평가 기능은 규칙·계산·저장·API·화면 전 계층을 병합하고 마이그레이션 둘을 적용했으며 실사용 검증과 지원 범위 확장을 추진합니다.
...
- **적격심사 정량평가 (2026-09-09, 동작)**: 공고 상세 페이지에 공고 기준 분석 카드를 붙였습니다. 판별 정본은 `raw_data.sucsfbidMthdNm` 문자열이고 별표 14종 하한율은 DB 550만 건 실측으로 확정했습니다(보험용역은 문서 47.994 대 실측 **47.995**). 가격배점 B 와 계수 k 와 통과점수 T 는 별표마다 다르고 공고 데이터에 없어 **사용자가 공고문 배점표를 입력**하며, 셋 중 하나라도 없으면 `MISSING_SCORE_TABLE` 로 점수만 차단하고 하한율 기반 최저 투찰금액과 예정가격 시나리오는 계속 제공합니다. 지원 범위는 일반용역 적격심사뿐이며 실측상 일반용역 공고의 약 6% 입니다. 남은 것은 여러 공고 실사용 검증과 기술용역·협상계약 확장입니다.
- **금액 집계 성능 (2026-09-09, 부분 완료)**: `GET /api/v1/bids/stats` 웜 레이턴시는 3.34ms(P95 3.80ms)로 실측했고 이 경로에 성능 우려는 없습니다. 다만 `base_amount` 전환의 주 수혜 경로인 `GET /api/v1/bids/compare-stats`(`agency_announce_top10`, 전환 전 웜 31.97초)는 **미측정**입니다. 캐시 보호를 받지 않아 콜드 조건에서 버퍼풀을 비우고 재야 하므로 측정 창을 따로 잡아야 합니다. 따라서 31.97초 병목의 해소 여부는 아직 근거가 없습니다.
