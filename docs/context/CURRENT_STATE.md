...
- **rag_segments_canonical**: RAG 구간 정본은 타임아웃 300초 96요청 전량 성공으로 canonical 게이트를 통과했으나 버퍼풀 콜드 조건은 재현하지 못했습니다.
...
- **RAG cold SQL (2026-09-07, 정본 확보·콜드 조건 미재현)**: 타임아웃 300초 재측정이 96요청 전량 성공으로 canonical 게이트를 통과했습니다(`rag_segments_canonical_20260907_t300.json`, sql 구간 콜드 P50 12.9ms·max 320.3ms). 다만 하네스의 is_cold는 버퍼풀 상태가 아니라 문항별 첫 반복이며 이 실행은 버퍼풀이 이미 채워진 뒤라, 버퍼풀 콜드에서 관측된 100초대 값(1차, q03·q25·q31 타임아웃)은 대체되지 않았습니다. DB 재기동 후 콜드 재현이 남았습니다.
