...
- **KB 색인 메모리 폭주 (2026-09-09, 해소)**: Wave AP 다섯 Task 를 병합한 뒤 두 조건에서 실측해 해소를 확인했습니다. 16GiB 조건은 피크 4,323MB 에 959초 완료, Docker 메모리를 운영 기본값 14,336MiB 로 되돌린 조건은 피크 1,027MB 에 82초 완료이며 두 경우 모두 컨테이너 소멸이 없고 `knowledge_base_status` 가 `ready` 로 갱신됐습니다. 오전 실패가 16GiB 에서도 재현됐으므로 통과 원인은 메모리 여유가 아니라 코드 수정입니다. `HEAVY_TASK_NAMES` 에 `run_schedule_catchup_task` 를 추가해 3시간 타임아웃을 적용했고, `nightly_schedule_task` 는 cron 등록에서 이미 `timeout=10800` 을 받고 있어 대상이 아님을 확인했습니다. 남은 것은 수집 공백 20260827~20260901 구간의 `backfill_from_g2b.py` 보충입니다.
