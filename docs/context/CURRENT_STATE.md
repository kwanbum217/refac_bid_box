...
- **kb_index_memory**: KB 색인 메모리 폭주는 Wave AP 전량 병합(4e40bd1) 후 재측정에서 16GiB 조건 피크 4,323MB 와 14GiB 원복 조건 피크 1,027MB 로 모두 정상 완료해 AP4 판정 기준 3항을 충족했습니다.
...
- **KB 색인 메모리 폭주 (2026-09-09, 해소)**: Wave AP 다섯 Task 를 병합한 뒤 두 조건에서 실측해 해소를 확인했습니다. 16GiB 조건은 피크 4,323MB 에 959초 완료, Docker 메모리를 운영 기본값 14,336MiB 로 되돌린 조건은 피크 1,027MB 에 82초 완료이며 두 경우 모두 컨테이너 소멸이 없고 `knowledge_base_status` 가 `ready` 로 갱신됐습니다. 오전 실패가 16GiB 에서도 재현됐으므로 통과 원인은 메모리 여유가 아니라 코드 수정입니다. `HEAVY_TASK_NAMES` 에 `run_schedule_catchup_task` 를 추가해 3시간 타임아웃을 적용했고, `nightly_schedule_task` 는 cron 등록에서 이미 `timeout=10800` 을 받고 있어 대상이 아님을 확인했습니다. catchup 로그가 안내한 수집 공백 20260827~20260901 은 실재하지 않음을 확인했습니다. 공고일 기준 2026-08-20~09-08 이 매일 채워져 있고(적은 날은 주말·공휴일), `backfill_from_g2b.py --dry-run` 이 그 구간에 대해 공고 0건 낙찰 0건을 보고했습니다. 로그의 경고는 데이터 공백이 아니라 수집 실행 간격 기준입니다.
