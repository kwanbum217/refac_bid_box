...
- **kb_index_memory**: KB 색인 메모리 폭주는 bf29f3e 병합 후에도 단일 색인 피크 10,762MB 로 재현되어 AP4 판정 기준 3항을 전부 미충족했고 조회 계층 페이지 전환과 재시도 폭풍 차단을 추진합니다.
...
- **KB 색인 메모리 폭주 (2026-09-09, 수정 실패 확정)**: 청크 스트리밍 수정 `bf29f3e` 는 병합됐으나 운영 규모 실측에서 폭주를 막지 못했습니다. 단일 색인 1회에 worker 피크 **10,762MB** 로 2026-09-08 의 10,509MB 곡선을 재현하고 소멸했으며, `knowledge_base_status` 도 `status=failed` 로 갱신되지 않았습니다(AP4 판정 기준 3항 전부 미충족). `_resolve_announcements` 의 전량 리스트 반환과 `existing_hashes` 536,002건 상주가 남은 원인이며 AP2(`task_e6226318e956`) 선행이 필요합니다. arq `in-progress` TTL 만료 시 같은 색인이 재발화하므로 AP5(`task_924bc554133d`) 도 함께 필요합니다.
