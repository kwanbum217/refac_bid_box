...
- **107건 (`value_differs` 소수부)**: `raw_data`가 소수점을 가지고 컬럼은 정수로 절단됨. 차이는 전수 1원 미만이며 합계 107원 이하(2,125조 원 대비 상대오차 5e-17). 새 파서 역시 과거 일관성을 위해 `ROUND_DOWN`을 적용하므로 신규 적재 데이터에서도 정합성이 유지됩니다.
