# Task 5428a2df9fe9 — main 병합 경로 전량 테스트 게이트 구축

> **작성일**: 2026-09-02
> **작업 ID**: task_5428a2df9fe9
...
1. **Fail-Closed 검증**:
   - 전량 테스트 통과 증거가 없거나, 파일이 손상되었거나, 증거의 `exit_code != 0`이거나, 증거의 커밋 SHA가 병합 대상 커밋(`MERGE_HEAD`)과 다르면 병합 커밋 생성을 즉시 차단(종료 코드 1)합니다.
2. **일반 커밋 속도 보존**:
   - 훅을 `pre-commit` 스테이지가 아닌 `pre-merge-commit` 스테이지에만 등록하여, 일반 작업 커밋 시 55초의 전량 테스트 비용이 발생하지 않습니다.
3. **Target Branch 제한 (`main` 전용)**:
...
| 파일 경로 | 변경 유형 | 설명 |
| --- | --- | --- |
| `scripts/premerge_full_suite_gate.py` | 신규 생성 | pre-merge-commit 게이트 및 전량 테스트 증거 생성/검증 도구 |
| `tests/test_premerge_full_suite_gate.py` | 신규 생성 | 모의 러너 기반 단위 테스트 (11건) |
| `.pre-commit-config.yaml` | 수정 | `pre-merge-commit` 스테이지에 `premerge-full-suite-gate` 로컬 훅 등록 |
| `docs/ops/git_branching_strategy.md` | 수정 | 병합 프로세스에 전량 테스트 증거 기록 및 게이트 검증 절차 문서화 |
| `docs/analysis/task_5428a2df9fe9.md` | 신규 생성 | 본 작업 분석 및 결과 보고서 |
...
- **검증 항목**:
  ...
  - 증거 커밋 해시 일치 및 `exit_code == 0` 시 정상 통과
...
- **결과**: `3209 passed, 35 skipped, 3 deselected, 311 warnings in 85.28s`
- **참고 사항**: 격리 워크트리 환경에는 원본 대용량 데이터 자산(`data/model_files/`, `chroma_db/`)이 배치되지 않으므로, `tests/test_data_preservation.py`의 자산 존재 검사 2건은 Capsule 사양(`ground_truth` #57)에 명시된 대로 `-m 'not data_assets'`로 제외되었습니다.
...
2. **커밋 단위 정합성 보장**: 테스트 증거가 특정 커밋 해시에 엄격히 바인딩되므로 이전 커밋의 낡은 증거(stale evidence)를 재사용하여 회귀를 통과시키는 행위가 불가능합니다.
3. **개발 생산성 유지**: 일반 작업 커밋 및 `feature` 브랜치 간 병합에는 게이트가 발동하지 않으므로 일상적인 개발 속도에 부정적 영향을 주지 않습니다.
