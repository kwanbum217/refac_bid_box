...
## 3. 더 넓은 불확실성 표시 — 기존 Interval 아티팩트 활용 방안
...
| --- | --- | --- |
| 집단별 성능 게이트 정의(지표·임계·근거) | 2.1, 2.2 | 충족 |
| 더 넓은 불확실성 표시 방법(기존 interval 아티팩트 활용 판단) | 3.1~3.3 | 충족 |
| 예측 보류·경고 기준(구체적 조건) | 4.1, 4.2 | 충족 |
| 사용자 화면 근거 데이터 부족 표시(템플릿 위치 명시) | 5.1~5.3 | 충족 |
| 집단별 드리프트 모니터링 PSI 설계 연계 | 6.1~6.3 | 충족 |
| 기각된 세 접근 미제안 명시 | 7 | 충족 |
| 미지 항목 별도 절 분리 | 8 | 충족 |
| 보고서 단일 파일(`servc_missing_lwlt_policy_20260902.md`), 코드 미수정 | 본 파일 | 충족 |
