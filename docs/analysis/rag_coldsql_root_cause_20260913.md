...
| 순서 | 작업 | 선행 조건 |
| :---: | --- | --- |
| 1 | q08 금액 집계(테이블 전체 스캔)의 대책. `(dminstt_nm, sucsf_bid_rate, sucsf_bid_amt)` 커버링 인덱스 크기 추정부터 | 없음 (DDL 은 합의) |
| 2 | 상한을 넘은 기관명의 해석 질의 낭비 제거. 넘을 것이 확실한 경우 해석 질의를 건너뛸 수 있는지 | 없음 |
| 3 | 소규모 테이블 콜드 측정 절차 보완 (호스트 캐시) | 없음 |
| 4 | `bid_results` 의 남은 `FTS_DOC_ID_INDEX`(82MB) 확인. 기각된 FULLTEXT 제거 뒤 숨은 열이 남은 것으로 보임 | 조사 후 판단 |
| 5 | 웜 회차 최대값 증가(9장) 재현 확인 | 없음 |
