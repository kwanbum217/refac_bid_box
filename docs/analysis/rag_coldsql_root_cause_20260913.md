...
경계 행 수는 10만~44만 사이 어딘가이며 확정하지 않았습니다. 이름 수 대신 일치 행 수로 판정하려면 해석 단계가
행 수를 함께 세야 하는데, 그 COUNT 는 기관명 인덱스 커버링이라 콜드에서도 1초 안팎입니다(위 표). 경계를
확정하기 전에는 도입하지 않습니다.

### 8.1 q08 의 28초 구간에 남은 선택지

| 선택지 | 기대 | 비용과 위험 |
| --- | --- | --- |
| `(dminstt_nm, bid_ntce_nm)` 복합 인덱스 | GROUP BY 가 인덱스만으로 끝남 | `bid_ntce_nm` 이 `varchar(500)` 라 키가 크고 5.5M 행 인덱스 크기·수집 쓰기 비용 실측 필요. DDL 합의 필요 |
| 기관명 단위 사전 집계 스냅샷 | 실시간 집계 제거 | compare-stats 스냅샷과 같은 방식. 기관명 조합이 많아 대상 범위 설계 필요 |
