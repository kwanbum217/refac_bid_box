# Task task_ad2486bbb6bc (Section K4): 워커 감시 자동화 및 Fail-Closed 강화 보고서

> **작성일**: 2026-09-01
> **Task ID**: `task_ad2486bbb6bc` (Section K4)
> **대상 브랜치**: `kwanbum217/orca-k4-sup`
...
- `start_worker_watch(repo)`:
  ...
  - 프로세스가 없거나 사망 상태이면 `subprocess.Popen([..., "orca_worker_watch.py", "--repo", ..., "--watch"], start_new_session=True)` 로 배경 기동 후 PID 파일 기록.
...
- `.claude/skills/orca-section-coordination/SKILL.md` 및 `.agents/skills/orca-section-coordination/SKILL.md` (3.2.1절):
...
- `.claude/skills/orca-section-coordination/SKILL.md` 와 `.agents/skills/orca-section-coordination/SKILL.md` diff 0건 (완전 일치).
