...
운영 서비스의 복구 목표 수준(RPO, RTO)은 2026-09-06에 정식 확정되었으며, 일 1회 정기 백업 스케줄과 분기 1회 복원 드릴(restore drill)을 통해 이를 준수합니다 ([`docs/context/CURRENT_STATE.md`](../context/CURRENT_STATE.md)).

| 지표 | 정의 | 목표 기준선 (SLA) | 확정일 | 비고 및 근거 |
| --- | --- | --- | --- | --- |
| **RPO** (Recovery Point Objective) | 허용 가능한 최대 데이터 유실 시점 간격 | **24시간** | 2026-09-06 | 일 1회 정기 스냅샷 백업 스케줄(`backup_schedule_task`)로 충족 |
| **RTO** (Recovery Time Objective) | 장애 발생 시점부터 서비스 정상화까지 허용 시간 | **4시간** | 2026-09-06 | 덤프 다운로드, DB import, 파일 아카이브 해제, 사후 G1 무손실 검증 소요 시간 실측 기준선 충족 |

> **확정 근거**: `docs/context/CURRENT_STATE.md`의 `rpo_rto` 기계 사실에 근거하여 RPO 24시간, RTO 4시간이 확정되었습니다. 정기 백업 스케줄을 통해 RPO 24시간을 보장하고, 단일 복구 단위 기반 복원 파이프라인 및 분기 1회 복원 드릴(restore drill) 검증을 통해 RTO 4시간 이내의 안전한 서비스 정상화를 보장합니다.
...
정기 백업 스케줄(`BACKUP_SCHEDULE_ENABLED=true`)이 매일 1회 실행되어 RPO 24시간 목표를 충족합니다.
...
- **G1 파일 검증 세부 구간 기록**: 파일 G1 보고서의 계측값(`step_timings`, `segments`)을 drill 보고서의 `g1_file_breakdown` 으로 옮겨, 해시한 파일의 저장소 상대 경로와 읽은 바이트, 파일·ChromaDB sqlite 집계·조회 구간별 소요 시간을 남깁니다. 계측값이 없는 구버전 보고서에는 필드를 만들지 않습니다.
