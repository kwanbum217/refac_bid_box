...
### 2.11 Servc 계약방식별 분리 모델 (세그먼트 서브모델)

**세 세그먼트 전부에서 분리가 단일보다 나빴습니다.** 2026-08-03 실측입니다.
...
`cntrct_mthd_nm` 과 `sucsfbid_mthd_nm` 을 범주형 특징으로 넣은 시점에 트리가 이미
세그먼트 분기를 학습하므로, 명시적으로 나누면 그 분기만 강제되고 학습 표본만
줄어듭니다. 2026-08-07 문서가 "두 번 실패했다" 로 재확인했습니다.

**2026-09-01 주의**: `servc_lwlt_missing_20260830.md` 가 이 설계를 대응 경로로 다시
권고하고 있었습니다. 기각 이력을 확인하지 않은 서술이며 같은 날 정정했습니다.
**분석 문서의 권고를 기각 목록과 대조하지 않고 착수하지 마십시오.**

### 2.12 Servc 낙찰하한율 결측 보전

**보전 경로가 존재하지 않습니다.** OOS 결측 1,356건은 100% 제도적 개념 부재이고
수집 누락은 0건입니다. DB 내 대체 필드와 외부 API 조인 모두 확보 0건이며, 추정
대입은 오차가 7~12%p 폭증해 기각됐습니다.

두 집단은 실제 낙찰률 분포가 다른 **별도 모집단**입니다(결측 94.95%/SD 5.19, 보유
88.67%/SD 2.62). 값을 채워서 좁혀질 차이가 아닙니다. 유효하게 남은 대응은 **결측
집단 전용 예측구간 관리** 하나입니다.

**2026-09-01 주의**: `CURRENT_STATE` 2.5 절이 개선 경로를 "낙찰하한율 결측 보전" 으로
적고 있었고, 그 서술을 근거로 조사가 다시 시작됐습니다. 같은 날 정정했습니다.
