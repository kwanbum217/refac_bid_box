...
`cerebras/gemma-4-31b` 는 TPM 초과 위험이 있고 OpenCode 무료 모델은 안정성과 컨텍스트 제약이 있어 주력·임계 경로에서 배제합니다. `deepseek-v4-flash-free` 는 2026-08-20 현재 opt-in 미승인으로 호출되지 않습니다.
...
| 모델 | 판정 | 근거 |
| --- | --- | --- |
| `opencode/deepseek-v4-flash-free` | **2026-08-20 opt-in 미승인으로 호출 불가. 삭제 아님** | 최신판이 중국 호스팅이라 워크스페이스 Go 설정에서 opt in 이 필요하다. opt in 하면 복구된다. 이전 판정은 주력 사용 가능이었다. 컨텍스트 1M, 추론 high. 테스트 계약 13건을 정확히 갱신했고 지시하지 않은 불변식 단정(`reviewer` 제외)까지 추가 |
