...
| `cerebras/gemma-4-31b` | **Orca 워커로 불가** | 무료 등급 분당 30K 토큰 상한. 아래 계산 참조 |
...
2026-08-18 실측에서 Orca 프리앰블과 3,847자 Capsule 만으로 28K 가 찼습니다.
**Orca 지시 체계 자체가 이 모델의 예산을 거의 다 씁니다.**

종전 기록(2026-08-17)은 이 제약을 "파일 수를 줄여도 해소되지 않는다" 로
적었는데 원인 지목이 틀렸습니다. 파일 수가 아니라 **누적 컨텍스트**가
문제이며, Capsule 을 10K 이하로 줄이면 도구 호출이 적은 작업은 가능합니다.
...
- **무료 모델에 Orca 표준 Capsule 을 그대로 주지 마십시오.** 컨텍스트 상한이
  작은 모델에는 Capsule 크기 자체가 작업 성패를 가릅니다. 배정 전에 상한을
  확인하고 사양을 그 안에 맞추십시오.
