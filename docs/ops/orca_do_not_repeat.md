...
`cerebras/gemma-4-31b` 는 TPM 초과 위험이 있고 OpenCode 무료 모델은 안정성과 컨텍스트 제약이 있어 주력·임계 경로에서 배제합니다. `deepseek-v4-flash-free` 는 2026-08-20 에 무료 풀에서 사라졌습니다.
...
| 모델 | 판정 | 근거 |
| --- | --- | --- |
| `opencode/deepseek-v4-flash-free` | **2026-08-20 소멸. 배정 불가** | 아래는 소멸 전 기록이다. 무료 풀 목록과 `opencode run` 양쪽에서 사라졌고 `opencode-go/` 유료 풀에만 남았다. 소멸 전 판정은 주력 사용 가능이었다. 컨텍스트 1M, 추론 high. 테스트 계약 13건을 정확히 갱신했고 지시하지 않은 불변식 단정(`reviewer` 제외)까지 추가 |
