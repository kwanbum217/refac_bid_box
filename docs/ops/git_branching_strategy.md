...
### 4.3 병합 묶기와 CI 비차단 확인 (2026-09-13 합의)

병합 한 번에 로컬 전량 테스트 약 2분과 GitHub CI 약 5.5분이 듭니다. 원격에는 `main` 만 푸시하므로 모든 병합이 전체 CI 를
돌립니다.

| 규칙 | 내용 |
| --- | --- |
| 병합 묶기 | 코드·문서·인수인계·`CURRENT_STATE` 의 `source_commit` 갱신을 작업 브랜치 하나에 넣고 한 번 병합합니다. 문서만의 별도 병합, `source_commit` 전용 병합을 만들지 않습니다 |
| `source_commit` | 병합 직전 `main` HEAD 값으로 **작업 브랜치에서** 갱신합니다. 병합 커밋을 `--amend` 로 고치지 않습니다(사전 병합 훅이 거부하고, 남은 스테이징이 로컬 규칙 검증을 속입니다) |
| 푸시 게이트 | `[ -z "$(git status --short)" ] && python3 scripts/validate_agent_rules.py --quiet && git push origin main` |
| CI 비차단 확인 | 푸시 뒤 CI 완료를 기다리지 않고 다음 작업을 진행합니다. **다음 병합 직전과 세션 종료 전에는 직전 `main` CI 결과를 반드시 확인**하고, 실패면 추가 병합을 멈추고 먼저 고칩니다 |

마지막 행의 확인을 건너뛰면 2026-09-11 처럼 `main` CI 가 확인 없이 연속 실패합니다(11회).
