...
| `batch-disable` | 배치 스레드 안에서만 GC 를 비활성화합니다. |
...
gen1 을 20, gen2 를 30 으로 올리면 generation-2 GC 가 발생하는 빈도가
기본 대비 약 1/3 로 줄어들어 지연 꼬리를 낮추면서도 GC 자체는 유지됩니다.
...
마이크로배치 스레드(`prediction-microbatch-*`) 안에서만 `gc.disable()` 을 호출합니다.
CPython 의 GIL(Global Interpreter Lock) 구조상 GC 는 스레드별로 제어가 가능합니다.
배치 스레드에서 GC 를 끄면 추론 중 generation-2 일시 정지가 사라집니다.
메모리 누증을 막기 위해 50 배치마다 `gc.collect()` 를 명시적으로 호출합니다.

**위험 요소**

| 위험 | 설명 |
| --- | --- |
| 스레드 누증 | gc.disable() 은 프로세스 전역이므로 다른 스레드도 영향을 받습니다. |
