...
기본 Medium에서 벗어나는 모든 Gemini Dispatch 전에는 Capsule의 `ground_truth` 또는
`why_now`에 아래 알림을 넣고, 코디네이터는 사용자 대화에도 같은 내용을 알립니다.

```text
[WORKER_MODEL_NOTICE]
- 대상 작업: <Task 범위>
- 변경: Gemini Flash Medium -> <Gemini Flash High | Gemini Flash Low>
- 사유: <high 위험도 또는 low 위험도 조건>
- 영향: <Medium 대비 사용량·품질·지연의 방향>
- 복귀: <해당 Dispatch 완료 즉시 Medium>
```
...
2. **Level 1 (결정론적 기계 검증)**: 10대 게이트(변경 파일, 범위, 테스트, 규칙, 린터, 리뷰 보고, gitignore, 커밋 메시지, 금지 lockfile, 명령 실재성)를 단일 게이트 도구 호출로 수행합니다.
   ```bash
