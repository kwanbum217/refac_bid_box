...
비교 불가는 조용한 기각이 아니라 경고 알림으로 나갑니다. 게이트가 판단을 못 한 상태이므로 담당자가 직접 봐야 합니다. `quantum_leap_v25_pro` 는 지금 이 상태이며, 해소하려면 서빙본의 지표를 측정해 기록하거나 담당자가 최초 승격을 명시적으로 수행해야 합니다.
