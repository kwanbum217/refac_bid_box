...
- 모든 컨테이너는 `user` 지시자로 비특권 UID/GID 로 실행됩니다.
  - `app`, `worker`, `meilisearch`: `1000:1000` (python:3.11-slim 기본 비특권 사용자)
  - `db` (mysql:8.0): `999:999` (mysql 사용자)
  - `redis` (redis:7-alpine): `999:999` (redis 사용자)
- **주의**: Dockerfile 에 `USER` 지시자가 없으므로 이미지 빌드 시점에는 root 로 파일이 생성됩니다. 볼륨 마운트 시 `:ro`(읽기 전용)로 마운트해 쓰기 권한 상승을 방지했습니다. 이미지 레벨에서 전용 사용자를 생성하려면 별도 Task 로 Dockerfile 을 수정하고 CI 빌드 검증을 거쳐야 합니다.
...
소스 코드와 정적 데이터(`src`, `ml_registry`, `data`, `chroma_db`)는 `:ro` 로 마운트합니다. 컨테이너 내부에서 코드나 모델 파일을 변경할 수 없어 공급망 공격 표면을 줄입니다. 쓰기가 필요한 경로(업로드, 임시 파일 등)는 별도 볼륨을 마운트하십시오.
