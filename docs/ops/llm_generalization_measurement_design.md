...
|------|---------------------|---------------|-----------|
| **numeric 팩트 정답률** | 61.8% (102개 중 63개) | **≥ 55.0%** (102개 중 56개) | 3회 반복 최악값 |
...
> **미해결 정책 충돌 (측정 전에 확정할 것)**: 위 numeric 통과 기준 55.0% 는 e4b
> 기준선 61.8% 보다 낮습니다. fixture 밖이 더 어렵다는 이유로 마진을 준 것으로
> 읽히나, 5.3 절은 "e4b v4 수준 아래로 떨어지면 승격 근거가 소멸한다" 고 서술해
> 서로 어긋납니다. 둘 중 어느 것이 실제 문턱인지 **측정 전에** 정하십시오. 이것은
> 사실 판단이 아니라 정책 결정이므로 코디네이터가 임의로 고르지 않았습니다.
...
```
IF (모든 필수_통과_기준_충족) AND (조건부_통과_기준_충족):
    결과 = "e2b_승격_유지"
ELSE:
    결과 = "e2b_승격_재검토_필요"  # e4b 복귀 또는 대체 모델 탐색
```

**중요**: v4 측정에서 e4b는 numeric 61.8%, 문항 통과 12/48였다. e2b가 fixture 밖에서 **e4b v4 수준 아래로 떨어지면** 승격 근거가 소멸한다.
...
| e2b만 측정하고 e4b는 생략해도 되나? | **안 됨**. 비교 기준선(e4b v4 fixture 밖 성능)이 없으면 판정 불가. |
