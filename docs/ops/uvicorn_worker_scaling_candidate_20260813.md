...
아니므로 워커 수에 비례해 총 메모리가 늘어난다고 예상하지만, 아래 세 수치는
측정 환경이 서로 다르므로 섞어 쓰지 않습니다.

| 항목 | 값 | 측정 환경 | 성격 |
| --- | --- | --- | --- |
| 프로세스당 RSS | 약 305~308MB | 로컬(비 Docker) 3워커 실험, 자식 프로세스 개별 측정 | 로컬 후보 추산의 근거 수치 |
| 로컬 3워커 합산 | 약 0.92GB | 위 로컬 실험값의 단순 합산(305~308MB × 3) | **로컬 후보 추산**. Docker 확정값 아님 |
| 현재 Docker 단일 앱 컨테이너 기준선 | 662.5MiB | `docker stats` (1워커, 현재 운영 중인 컨테이너) | **Docker 실측 기준선** |
| Docker 3워커 총 메모리 | 미측정 | - | 본 작업에서 공유 Docker/DB를 점유하지 않기 위해 실제 재기동·측정을 수행하지 않음 |

로컬 3워커 합산(약 0.92GB)은 컨테이너 밖에서 프로세스 수만 바꿔 본 실험의
RSS를 그대로 더한 **추산치**이며, Docker 컨테이너의 cgroup 메모리 계정 방식·
공유 라이브러리 매핑·page cache 공유 여부를 반영하지 않았으므로 Docker
3워커 총량과 다를 수 있습니다. 현재 확정적으로 아는 값은 Docker 단일 앱
컨테이너의 `docker stats` 기준선 662.5MiB뿐이며, Docker 3워커 총 메모리는
[4절](#4-승격-게이트)의 게이트로 실제 재기동 후 측정해야 합니다. 컨테이너
`mem_limit`은 현재 `docker-compose.yml`에 설정되어 있지 않습니다.
...
| 메모리 | Docker 3워커 구성의 앱 컨테이너 총 메모리를 실제로 측정하고(로컬 0.92GB 추산치로 대체하지 않음), 호스트 여유 메모리가 그 실측값 + 나머지 스택(DB/Redis/Meilisearch/Ollama)을 함께 수용함을 확인 | `docker stats --no-stream <app 컨테이너>`로 3워커 재기동 후 총 메모리 측정. 현재 Docker 단일 앱 컨테이너 기준선(662.5MiB)과 비교해 증가분 확인 |
