...
| --- | --- | ---: | --- |
| 스냅샷 검증 | 1분 | 2.0초 | restore drill 보고서의 단계별 소요 시간 |
| 아카이브 해제 | 1분 | 15.5초 | restore drill 보고서의 단계별 소요 시간 |
| DB import | 70분 | 1,391.6초 | restore drill 보고서의 단계별 소요 시간 |
| G1 무손실 검증 (파일 3.2초 + DB 7.9초) | 1분 | 11.1초 | restore drill 보고서의 단계별 소요 시간 |
| 정리 | 1분 | 3.7초 | restore drill 보고서의 단계별 소요 시간 |
| 서비스 정상화 (app·meilisearch 재생성부터 첫 200 응답) | 1분 | 15.1초 (3회 21.7·15.1·15.1초의 중앙값) | `docker compose up -d --force-recreate --no-deps meilisearch app` 후 health healthy 와 `/accounts/login/` 200 까지 |
| Meilisearch 읽기 모델 전체 재색인 | 42분 | 839.3초 (클라이언트 적재 581.9초 + 서버 색인 완료 대기 257.4초) | 빈 임시 Meilisearch(v1.14)에 `scripts/sync_search_index.py` 인자 없이 전체 동기화한 뒤 `/tasks` 의 enqueued·processing 이 0 이 될 때까지 |
