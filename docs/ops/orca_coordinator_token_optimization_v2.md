...
### 9.2.1 첫 실사용 결과 (2026-08-15)

**Reviewer 계층은 작동하지만 결함을 찾지 못했습니다.** 상세는
[`orca_v2_reviewer_plane_20260815.md`](orca_v2_reviewer_plane_20260815.md) 입니다.

| 항목 | 결과 |
| --- | --- |
| `ORCA_REVIEW_DONE_V2` 계약 도달 | 작동 |
| Capsule 격리 (2.9 규약 적용) | 작동. `read_files` 초과 0건 |
| diff 예산 분할 (31,524자 -> 12,048 + 19,476) | 작동 |
| **결함 검출** | **미달. 실재 결함 3건을 놓치고 `pass`** |

놓친 결함은 `subprocess.run` 의 `timeout` 부재, 줄을 넘는 정규식 매칭, 대상 부재를
통과로 처리하는 예산 검사입니다. 셋 다 담당 Reviewer 의 `review_priorities` 안에
있었으므로 범위 문제가 아니라 검토 깊이 문제입니다.

**따라서 Reviewer 의 `pass` 를 근거로 10장 Level 3 을 줄이지 마십시오.** 이번
실행은 Level 3 을 줄이면 결함이 통과한다는 것을 보여줍니다.

### 9.2.2 개선과 감도 측정 (2026-08-15)

첫 실사용 실패 후 계약을 v2.1.0 으로 고치고 seeded defect 로 효과를 측정했습니다.
상세는 [`orca_v2_reviewer_sensitivity_20260815.md`](orca_v2_reviewer_sensitivity_20260815.md)
입니다.

| 지표 | v2.0 서술형 | v2.1 체크리스트 (같은 모델) |
| --- | --- | --- |
| `verdict` | `pass` | **`fail`** |
| 체크리스트 정확도 | 항목 없음 | **4/4** |
| 자유 발견 | — | 1/2 |
...
나왔습니다 — 한 보고가 유효한 JSON 이 아니었고, 두 보고 모두 `blocking_issues` 에
체크리스트 `id` 를 넣지 않아 대응 관계를 기계로 이을 수 없었습니다.

**남는 한계**: 체크리스트에 없던 결함 2개 중 1개를 두 모델 모두 놓쳤습니다.
체크리스트는 **아는 위험을 위임하는 수단**이며 미지의 결함 탐색은 여전히
코디네이터 몫입니다. Level 3 은 유지합니다.
...
| 단계 | 산출물 | 범위 |
| --- | --- | --- |
| 정적 | [`orca_v2_cross_cli_validation_20260815.md`](orca_v2_cross_cli_validation_20260815.md) | 5개 CLI 진입점 파일·설정 정합성, 검증기 기각 동작, 스킬 미러 바이트 일치 |
| **실행** | [`orca_v2_runtime_smoke_20260815.md`](orca_v2_runtime_smoke_20260815.md) | 실제 워커에서 Capsule -> `worker_done` 흐름, 금지 문서 재독 0건, acceptance 독립 재검산 |

**정적 검증만으로는 20장 10번을 충족할 수 없습니다.** 설정이 완벽해도 지시가
워커에 도달하지 않는 실패가 실재합니다(2026-08-14 세션에서 `dispatch --inject`
3회 실패, 설정 정합성은 그때도 통과 상태).

실행 검증의 핵심 결과입니다.

| 지표 | 값 |
| --- | --- |
| Capsule 크기 | 2,941자 (5장 예산 4,000자 이내) |
| `allowed_read_files` 대비 실제 Read | **4개 = 4개, 정확히 일치** |
| 금지 문서(README/SKILLS/AGENTS/설계서/handoff) 재독 | **0건** |
| acceptance 독립 재검산 | 6개 전부 통과 |

근거는 워커의 자기 보고가 아니라 코디네이터가 터미널 출력에서 직접 센 `Read()`
호출 수입니다.

**함께 확인된 한계**: `allowed_read_files` 는 지켜지지 않았습니다. 워커가 8개를
읽어 허용 4개를 넘었고, 그중 2개는 다른 Task 의 파일이었습니다. 원인은 Capsule 을
공유 스크래치패드에 두고 그 경로를 허용 목록에 넣지 않은 **코디네이터의 배치
오류**입니다. 조치는 `orca_task_capsule_v2.md` 2.9 절 배치 규약입니다.

**이 필드는 강제 장치가 아니라 지시입니다.** 준수는 `worker_done` 의 `read_files`
로 사후 확인해야 하며, 그 요구가 계약에 있는 이유가 이것입니다. 이번 검증에서
**워커의 자기 보고가 코디네이터의 관찰보다 정확했습니다.**
