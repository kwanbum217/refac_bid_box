...
| `verification_commands` | list[string] | 예 | 워커가 로컬에서 실행하고 통과해야 하는 검증 명령어. Level 1 게이트 3 이 그대로 실행하므로 허용 목록(`uv run pytest ...`, `npm ci`, `npm run <script>`)만 쓸 수 있고, `frontend/` 를 고치면 frontend 검증이 반드시 포함되어야 합니다 |
