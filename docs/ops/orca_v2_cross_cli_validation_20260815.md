...
> **작성일**: 2026-08-15
> **버전**: v1.0.0
> **태스크 ID**: task_58ae6734cb77
> **디스패치 ID**: ctx_7e6d30ab3791
...
본 보고서는 Orca Task Capsule v2 체계에서 병합된 규칙 자동 로드 계약이 지원 대상 5대 CLI 진입점(Codex, opencode, Antigravity, Claude Code, Cursor) 및 결정론적 검증기(`scripts/validate_agent_rules.py`)에서 정상 작동하는지 실측 검증한 결과입니다.

주요 검증 범위:
1. 5대 AI 코딩 CLI 진입점의 단일 진실 원천(`AGENTS.md`) 참조 정합성
...
5개 CLI 환경의 진입점 파일과 설정값을 실측 확인하였습니다.
...
| **Antigravity CLI** | `.antigravity/rules.md` | 요약본 로드 | 6,167자 (12,000자 상한 대비 5,833자 여유), 7개 필수 키워드 완비 | **통과** |
...
테스트 결과: 12개 단위 테스트 케이스 전량 예상과 정확히 일치하여 정상 기각 및 정상 수락이 확인되었습니다 (12 passed, 0 failed).
...
1. **실제 진입점 참조 검증 완료**: Codex, opencode, Antigravity, Claude Code, Cursor 등 5대 CLI 진입점이 단일 진실 원천(`AGENTS.md`) 및 얇은 진입점 계약에 부합함을 실측 확인.
2. **레거시 이중 주입 기각 검증 완료**: `opencode.json` 내 `SKILLS.md` 이중 주입, 비정상 타입, `AGENTS.md` 내 `@SKILLS.md` 레거시 참조 등 12개 테스트 케이스 전량 정상 기각/수락 확인.
