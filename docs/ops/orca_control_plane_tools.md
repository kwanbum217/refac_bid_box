...
| **`coordinator-capsule`** | `--slug <id>`<br>`--objective <text>`<br>`--scope <path>` (반복)<br>`--out <path>`<br>`--json` | `--slug` (필수)<br>`--objective` (필수)<br>`--scope` (필수, 1개 이상) | 워커 없이 코디네이터가 직접 쓴 브랜치용 최소 `ORCA_TASK_CAPSULE_V2` 를 만듭니다. **Orca Task 를 만들지 않고 `orca` CLI 를 호출하지 않습니다.** `--out` 기본값은 `.orca/capsules/coord_<slug>/capsule.yaml` 이며 이미 있으면 덮어쓰지 않고 종료 코드 2 로 거부합니다. |
