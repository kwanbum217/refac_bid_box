        ...
        if col in CATEGORICAL_FEATURES or not pd.api.types.is_numeric_dtype(df_feat[col]):
            ...
            if series.empty:
                features_summary[col] = {
                    ...
                    "histogram": {"bins": 10, "counts": [0] * 10, "bin_edges": [0.0] * 11},
                }
                continue
            ...
            counts, bin_edges = np.histogram(series.to_numpy(), bins=config["num_buckets"])
            features_summary[col] = {
                "type": "numeric",
                "min": min_v,
                "max": max_v,
...
def check_dataset_drift(
    ...
    하나라도 임계치를 넘으면 TRIGGER_RETRAIN (판정 상태 DRIFT_DETECTED)이 되며,
    표본 부족 시 INSUFFICIENT_DATA 를 반환합니다.
    """
