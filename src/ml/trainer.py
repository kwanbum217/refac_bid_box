...
# 뿐이었습니다. 63 -> 255 로 MAE 1.2829 -> 1.2652, 0.5%p 적중 60.57% -> 60.99%.
#
# **그러나 255 는 채택하지 않았습니다.** 재학습·승격 후 운영 API 경로에서 같은
# 표본 1000건으로 겨루자 MAE 는 0.8% 좋아지고 RMSE 는 1.3% 나빠져 정확도가
# 판별되지 않았고, 예측 구간 폭만 1.423 -> 1.583%p 로 11.2% 넓어졌습니다.
# 255 리프가 학습 구간에 더 맞으면서 분위 구간이 좁아지자 등각예측 배율이
# 1.163 -> 1.254 로 커졌고, 그 증가가 좁아진 폭을 상쇄하고 남았기 때문입니다.
# 구간 폭은 사용자가 화면에서 직접 보는 값이므로 확인되지 않는 정확도 이득과
# 바꾸지 않습니다. 근거는 docs/design/servc_hyperparam_search_20260804.md 6장.
#
