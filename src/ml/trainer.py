...
#
# alpha 는 0.45 입니다. 0.5(조건부 중앙값)가 최적일 것이라는 가정을 2026-08-10 에
# 실측으로 뒤집었습니다. 0.35~0.58 을 시드 3개로 훑으면 0.45 를 꼭짓점으로 하는
# U 자형이고, 2025년 홀드아웃에서 MAE 1.2407 -> 1.2323, 0.5%p 적중 62.2995% ->
# 62.4642% 로 여섯 지표가 모두 개선됩니다.
#
# 편향을 근거로 방향을 정하지 마십시오. 현행 편향 -0.0477 을 과소예측으로 보고
# alpha 를 올렸더니 전 지표가 나빠졌고, 편향이 -0.2128 로 네 배 커지는 0.45 가
# 최선이었습니다. 낙찰률 분포가 위로 꼬리를 끌어 최적 예측점이 조건부 중앙값보다
# 아래에 있습니다. 평균 편향은 그 사실의 부산물입니다.
# 근거: docs/design/servc_quantile_alpha_20260810.md
CATEGORY_HYPERPARAMS = {
    "Servc": {"lightgbm": {"num_leaves": 255, "objective": "quantile", "alpha": 0.45}}
}
