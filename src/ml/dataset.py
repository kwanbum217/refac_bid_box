...
4. 명시적 비예가 공고의 학습 프레임 배제 (수정 금지).
   ...
   ``sucsf_bid_rate IS NOT NULL`` 조건과 후속 ``dropna()`` 에서 대부분 배제된다.

   이는 의도된 설계는 아니나 부작용으로 배제된 것이며, 결과적으로 옳은 동작이다:
   - 비예가 공고는 예정가격을 작성하지 않는 제도이므로 낙찰률(= 낙찰금액 /
     예정가격)의 분모가 존재하지 않는다.
   - 학습 데이터에 잔존했던 2,405건(0.47%)은 국지적 표기 관행의 오염된(contaminated) 데이터로,
