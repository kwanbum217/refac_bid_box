...
class AutomationSubscription(Base):
    ...
    filter_json = Column(JSON, nullable=True, comment="필터 조건")
    schedule_cron = Column(String(100), nullable=True, comment="스케줄 Cron")
...
class KnowledgeBaseStatus(Base):
    ...
    last_pipeline_run_id = Column(String(100), nullable=True, comment="마지막 파이프라인 실행 ID")
...
class PipelineExecution(Base):
    ...
    stage_name = Column(String(100), nullable=True, comment="현재 스테이지명")
    stage_status = Column(String(50), nullable=True, comment="현재 스테이지 상태")
    ...
    metrics_json = Column(JSON, nullable=True, comment="실행 지표")
    raw_status_payload = Column(JSON, nullable=True, comment="원본 상태 페이로드")
    logs_summary = Column(Text, nullable=True, comment="로그 요약")
    external_url = Column(Text, nullable=True, comment="외부 실행 URL")
