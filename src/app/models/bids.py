...
class BidResult(Base):
    ...
    bid_ntce_nm: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="입찰공고명"
    )
    bid_ntce_no: Mapped[str] = mapped_column(String(50), nullable=False, comment="입찰공고번호")
    bid_ntce_ord: Mapped[str] = mapped_column(
        String(10), nullable=False, default="00", comment="입찰공고차수"
    )
    bidwinnr_nm: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="낙찰업체명"
    )
    sucsf_bid_amt: Mapped[int | None] = mapped_column(BigInteger, nullable=True, comment="낙찰금액")
    sucsf_bid_rate: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 4), nullable=True, comment="낙찰률"
    )
    rl_openg_dt: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="개찰일시"
    )
    dminstt_nm: Mapped[str | None] = mapped_column(String(200), nullable=True, comment="수요기관명")
...
class BidAnnouncement(Base):
    ...
    ntce_instt_nm: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="공고기관명"
    )
    dminstt_nm: Mapped[str | None] = mapped_column(String(200), nullable=True, comment="수요기관명")
    base_amount: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="기초금액(사업예산)"
    )
    presmpt_prce: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="원본 참고금액"
    )
    bid_ntce_dt: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="입찰공고일시"
    )
    bid_clse_dt: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="입찰마감일시"
    )
    openg_dt: Mapped[datetime | None] = mapped_column(DateTime, nullable=True, comment="개찰일시")
    ntce_kind_nm: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="공고종류명"
    )
    bid_methd_nm: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="입찰방식명"
    )
    cntrct_mthd_nm: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="계약방법명"
    )
    category: Mapped[str] = mapped_column(
        String(10), nullable=False, default="Thng", comment="업무구분(Thng/Cnstwk/Servc/Frgcpt)"
    )
...
class BidDatasetSummary(Base):
    ...
    total_amount: Mapped[Decimal] = mapped_column(
        Numeric(30, 0), nullable=False, default=0, comment="전체 금액 합계"
    )
    avg_rate: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 4), nullable=True, comment="평균 낙찰률"
    )
    source_latest_collected_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="원본 최신 수집 시각"
    )
...
class BidRankingSnapshot(Base):
    ...
    dataset: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="집계 대상 (announcement/result)"
    )
    dimension: Mapped[str] = mapped_column(String(30), nullable=False, comment="집계 축 컬럼명")
    # 전체 집계는 빈 문자열로 둡니다. NULL 은 유니크 제약에서 중복을 허용해 못 씁니다.
    category: Mapped[str] = mapped_column(
        String(10), nullable=False, default="", comment="업무구분 (전체는 빈 문자열)"
    )
    rank: Mapped[int] = mapped_column(BigInteger, nullable=False, comment="순위 (1부터)")
    label: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="집계 축 값")
    metric_count: Mapped[int] = mapped_column(BigInteger, nullable=False, default=0, comment="건수")
    rebuilt_at: Mapped[datetime] = mapped_column(
        DateTime,
        nullable=False,
        default=utcnow,
...
class InstitutionWinRateStat(Base):
    ...
    id: Mapped[int] = mapped_column(PKBigInteger, primary_key=True, autoincrement=True)
    institution_name: Mapped[str] = mapped_column(String(200), nullable=False, comment="수요기관명")
    # 전체 집계는 빈 문자열로 둡니다. NULL 은 유니크 제약에서 중복을 허용해 못 씁니다.
    category: Mapped[str] = mapped_column(
        String(10), nullable=False, default="", comment="업무구분 (전체는 빈 문자열)"
    )
    sample_count: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, comment="집계에 쓴 낙찰 건수"
    )
    avg_rate: Mapped[Decimal] = mapped_column(
        Numeric(10, 4), nullable=False, comment="평균 낙찰률 (퍼센트)"
    )
    ewm_rate: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 4),
        nullable=True,
