...
# --------------------------------------------------------------------------- #
# 8. 별표 14종 전량 회귀 테스트: 각 별표 유형의 공고 분석 결과를 검증한다
#    기존은 별표 1(시설분야용역) 만 검증되어 있었고 나머지 13종과 차단 경로는
#    실제 공고에서 어떻게 나오는지 확인되지 않았다.
# --------------------------------------------------------------------------- #

STAR_RAW_OVERRIDES: list[tuple[str, dict, str, float, float | None]] = [
    (
        "별표1_시설분야",
        {
            "sucsfbidMthdNm": "시설분야용역 적격심사 추정가격 5억원 미만",
    ...
    (
        "별표2_보험",
        {"sucsfbidMthdNm": "보험용역 적격심사 추정가격 5억원미만", "sucsfbidLwltRate": "47.995"},
    ...
    (
        "별표12_2억미만",
        {"sucsfbidMthdNm": "추정가격 2억원 미만인 용역", "sucsfbidLwltRate": "87.745"},
    ...
    (
        "별표13_2억이상5억미만",
        {"sucsfbidMthdNm": "추정가격 5억원 미만 2억원 이상인 용역", "sucsfbidLwltRate": "86.745"},
    ...
    (
        "별표14_15억이상30억미만",
        {"sucsfbidMthdNm": "추정가격 30억원 미만 15억원 이상인 용역", "sucsfbidLwltRate": "82.995"},
...
@pytest.mark.parametrize(
    ("scenario_name", "raw_overrides", "expected_rule_id", "expected_lwlt_rate", "base_amount"),
    [
        (name, overrides, rule_id, rate, amount)
        for name, overrides, rule_id, rate, amount in STAR_RAW_OVERRIDES
    ],
)
def test_each_star_analyze_returns_correct_rule(
    client,
    isolated_db,
    as_user,
    scenario_name,
    ...
    """별표 1-14 각 유형의 공고를 analyze 할 때 올바른 rule_id와 하한율이 반환된다.
    배점표가 없어 MISSING_SCORE_TABLE 차단이 발생하더라도 규칙 판별 정보는 정식 필드로 전달되어야 한다."""
    as_user(10)
    bid = _create_bid(isolated_db, base_amount=base_amount, raw_overrides=raw_overrides)

    response = client.post(ANALYZE_URL, json=_analysis_payload(bid.id))

    assert response.status_code == 200, f"[{scenario_name}] {response.text}"
    payload = response.json()
    # 규칙 판별 정보는 배점표 차단과 무관하게 정식 필드로 전달된다
    assert payload["rule_id"] == expected_rule_id, (
        f"[{scenario_name}] 기대 rule_id={expected_rule_id}, 실제={payload['rule_id']}"
    )
    assert payload["lower_bound_rate"] == pytest.approx(expected_lwlt_rate), (
        f"[{scenario_name}] 기대 하한율={expected_lwlt_rate}, 실제={payload['lower_bound_rate']}"
    )
    # ALL 14 별표에 대해 하한율과 A값 반영 투찰금액이 함께 전달되어야 A값 미반영 오류 방지
    assert payload["a_value_amount"] == 100_000_000, (
        f"[{scenario_name}] a_value_amount={payload.get('a_value_amount')}"
    )
    assert payload["min_bid_amount_with_a"] is not None, (
        f"[{scenario_name}] min_bid_amount_with_a 가 None 입니다"
    )
