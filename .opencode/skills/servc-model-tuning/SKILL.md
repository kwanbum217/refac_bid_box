...
| 미사용 제도 플래그 3종 특징 추가 (indstrytyLmtYn, cmmnSpldmdMethdNm, dsgntCmptYn) | 잔차 편향은 실재했으나(indstrytyLmtYn 집단 간 0.42%p, t +19.4, 3/3 연도 재현) 세 분할 홀드아웃에서 MAE 차이 -0.0177 / -0.0006 / -0.0022, **평균 -0.0068 로 분할 산포 0.0074 안이고 표준편차 0.0094 가 평균보다 큼.** 0.5%p 적중률은 2025년에서 부호 반전(-0.0947%p). 수준 분포가 원인으로 유력하며 dsgntCmptYn 은 Y 가 1.9%(17,060건) 라 t -5.2 가 유의해도 전체 MAE 를 움직일 질량이 없고 cmmnSpldmdMethdNm 은 최빈값 하나가 76% | `servc_flag_features_holdout_20260811.md` |
| `prdctClsfcLmtYn` 을 그대로 특징화 | 2025-01 이 **실질 구성 변화**임이 확정됨. 빈 문자열 전 구간 0건, 키 부재 0.03% 이하로 표기 이동 흔적이 없고 Y 92.10% 가 한 달 만에 26.72% 로 이동. Y-N 낙찰률 차이도 -7.14 -> -2.76 으로 축소되어 라벨의 의미 자체가 바뀜. 10년 내내 88~92% 였으므로 예외 구간 종료가 아니라 새 경계. 체제 지시자 없이 넣으면 두 체제를 한 수준으로 섞음 | `servc_flag_notation_verdict_20260811.md` |

### 잔차 편향이 있다는 것과 그 정보가 새롭다는 것은 다른 명제입니다

2026-08-11 에 이 구분을 대가를 치르고 배웠습니다. 제도 플래그 3종은 현행 잔차에
세 연도 일관된 편향을 남겼고 본페로니 임계도 넘겼습니다. 그런데 특징으로 넣으니
홀드아웃 평균 -0.0068 로 분할 산포 안이었습니다.

t 가 크다는 것은 **그 필드로 잔차를 가를 수 있다**는 뜻이지, **기존 특징이 그
정보를 모른다**는 뜻이 아닙니다. 기존 35종에 이미 상당 부분 들어 있으면 잔차
편향은 보이면서 추가 기여는 없습니다.

잔차 편향은 후보를 좁히는 데는 쓸모가 있지만 **기여의 예고가 아닙니다.** 편향
크기로 개선 폭을 기대하지 마십시오. 다음 후보를 고를 때는 편향 유의성과 함께
**수준 분포의 질량**을 먼저 보십시오. 소수 수준에 몰린 필드는 t 가 유의해도
전체 지표를 움직이지 못합니다.

### 원천 표기 변경과 실질 변화를 가르는 법

값 존재율을 "비어 있지 않은 문자열" 로 재면 **표기 변경이 체제 전환으로
보입니다.** 하한율에서 실제로 그랬고 하루치 판단을 그르쳤습니다. 원천이 부재를
문자열 `'0'` 으로 적다가 빈 문자열로 바꿨을 뿐인데 값 존재율이 100% 에서 38.3%
로 떨어진 것처럼 보였습니다.

최소한 네 상태로 나누어 세십시오.

    키 부재 / 빈 문자열 / 부재를 뜻하는 표기('0', 'N' 등) / 실질 값

구성비만으로 갈리지 않으면 **라벨의 예측적 의미가 보존되는지**를 보십시오.
집단별 낙찰률 차이가 경계를 넘어 유지되면 표기 이동이고, 차이 자체가 줄면 실질
변화입니다. `rbidPermsnYn` 은 Y-N 차이가 8년간 -1.68~-2.09 로 유지되어 표기
이동, `prdctClsfcLmtYn` 은 -7.14 에서 -2.76 으로 줄어 실질 변화였습니다.
...
**이 확인은 학습 프레임이 갱신됐을 때만 의미가 있습니다.** 2026-08-13 재확인은
2026-08-10 결과와 소수점까지 동일했는데, parquet 이 2026-08-03 에 고정돼 그
사이 표본이 한 건도 들어오지 않았기 때문입니다. 기본 Compose 는
`ML_WEEKLY_RETRAIN_ENABLED=false` 라 자동 해소 경로가 꺼져 있습니다. **재개
조건을 재기 전에 feature store 가 언제 갱신됐는지부터 보십시오.**

같은 재확인에서 신제도 MAE 가 대조군보다 +0.0837 나빠 보였으나, 원인은 모델이
아니라 하한율 결측 비중이 33.27% 에서 39.11% 로 오른 표본 구성이었습니다.
대조군 구성으로 표준화하면 -0.0156 으로 부호가 뒤집힙니다. **구간 간 MAE 를
비교할 때는 집단 구성을 먼저 통제하십시오.** 근거는
`servc_regime_reopen_check_20260813.md` 입니다.
...
| 최신 데이터 재학습 (특징·하이퍼파라미터 불변, 2026-09-14 개찰분까지) | 표본 외 최신성 쌍대 8주 MAE -0.0255 (t=-8.12). 운영 경로는 비회귀(MAE -0.0055 t=-1.34, 구간 폭 -0.1004 t=-13.91). 운영 쌍대는 두 모델 모두 학습한 구간이라 최신성을 못 잰다는 점을 근거로 사용자 승인 후 승격 | `v_20260915_133523_756` |
