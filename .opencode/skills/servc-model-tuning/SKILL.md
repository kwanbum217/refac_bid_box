...
| 후보 | 홀드아웃 결과 | 남은 절차 | 근거 |
| --- | --- | --- | --- |
| quantile 아래 `min_child_samples` 160 / `learning_rate` 0.03 / `n_estimators` 2000 / `subsample_freq` 5 | MAE 1.2562 -> 1.2525, 0.5%p 적중 +0.20%p, 시드 3개 전부 일관 | 운영 경로 쌍대 검정 9,000건 + 집단별 | `servc_hyperparam_quantile_20260807.md` |

`subsample_freq=5` 는 기여가 0.0004 로 시드 표준편차의 절반이고 huber 아래에서
기각된 이력이 있습니다. 조합이 운영에서 우세하면 이 축만 빼고 재확인하십시오.

**`n_estimators` 는 운영에서 상한으로만 작동합니다.** `_train_lightgbm` 이
`early_stopping(10)` 을 걸기 때문이며 탐색 스크립트는 걸지 않습니다. 조합이
유효한 이유는 학습률을 낮추면 조기 종료가 늦게 걸리기 때문이지 2000 그루를
쓰기 때문이 아닙니다.
