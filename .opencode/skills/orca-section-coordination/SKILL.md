...
위임하기로 정했으면 **사양이 코디네이터 비용을 결정합니다.** 사양을 자족적으로 쓰고 `README.md`·`AGENTS.md`·`SKILLS.md` 재독을 금지하며, 확정 사실은 "재조사 불필요" 로 명시하고, 보고 항목(커밋 수·해시, 회차별 값, 대표값, 판정, 차단 사유)을 지정합니다. 수치는 코디네이터가 한 명령으로 재계산할 수 있는 형태로 요구하고, 원시 출력은 워커 문서에 두어 코디네이터 컨텍스트에 넣지 않습니다.
